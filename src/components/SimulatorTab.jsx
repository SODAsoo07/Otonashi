...
export const SimulatorTab = ({ audioContext, files, onAddToRack }) => {
    ...
    const renderPreviewAndDraw = useCallback(async () => {
        ...
        const centerY = h / 2 + 35; const amplitude = h * 0.08;
        for(let i=0; i<w; i++) {
    ...
    }, [playHeadPos, manualPose, dragPart, getInterpolatedValue]);
    ...
    return (
        <div className="flex-1 flex flex-col p-4 gap-4 animate-in fade-in overflow-hidden font-sans bg-slate-50 font-bold" onMouseUp={() => setDragPart(null)}>
            <div className="flex-[3] flex gap-4 min-h-0 overflow-hidden font-sans">
                <div className="flex-1 bg-white rounded-2xl border border-slate-300 relative overflow-hidden shadow-sm flex flex-col">
                    <div className="flex-1 relative flex items-center justify-center p-4 bg-slate-100/50">
                        <svg viewBox="0 0 400 400" className="w-full h-full max-w-[380px] drop-shadow-2xl">
                            ...
                            {/* 입술 조작 시각화 가이드 영역 */}
                            <ellipse cx={375} cy={225} rx={45} ry={65} fill="rgba(34, 197, 94, 0.08)" stroke="rgba(34, 197, 94, 0.4)" strokeDasharray="4 2" className="pointer-events-none"/>
                            <text x={375} y={150} textAnchor="middle" fill="#16a34a" fontSize="11" fontWeight="black" className="pointer-events-none select-none font-sans uppercase tracking-tighter">입술 조작 영역</text>
                            
                            {/* 입술 핸들 */}
                            <ellipse cx={350 + liveTract.lipLen * 20} cy="225" rx={15 + liveTract.lipLen * 20} ry={10 + liveTract.lips * 35} fill="#db2777" opacity="0.85" className="cursor-move hover:opacity-100" />
                        </svg>
                        <div className="absolute inset-0"
                            ...
                            onMouseDown={(e) => {
                                ...
                                // 입술 조작 감지 범위 확대
                                if (nx > 0.7 && ny > 0.25 && ny < 0.75) setDragPart('lips');
                ...
                <div className="w-72 bg-white/40 rounded-2xl border border-slate-300 p-3 flex flex-col gap-4 overflow-y-auto custom-scrollbar font-bold font-sans">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-1 mb-2 font-sans"><div className="flex justify-between text-xs text-slate-700 font-black"><span>음량</span><span>{Math.round(liveTract.volume * 100)}%</span></div><input type="range" min="0" max="2" step="0.01" value={liveTract.volume} onChange={e => { setManualPose(true); setLiveTract(prev => ({ ...prev, volume: Number(e.target.value) })); }} className="w-full h-1.5 bg-slate-300 rounded-full accent-emerald-500 font-sans font-bold"/></div>
                        {[{id:'lips',label:'입술 열기'},{id:'lipLen',label:'입술 길이'},{id:'throat',label:'목 조임'},{id:'nasal',label:'비성'}].map(p => (<div key={p.id} className="space-y-1 font-sans"><div className="flex justify-between text-xs text-slate-500 font-black font-sans font-bold"><span>{p.label}</span><span>{Math.round(liveTract[p.id]*100)}%</span></div><input type="range" min="0" max="1" step="0.01" value={liveTract[p.id]} onChange={e=>{setManualPose(true); setLiveTract(prev=>({...prev,[p.id]:Number(e.target.value)}));}} className="w-full h-1 bg-slate-300 rounded-full accent-[#209ad6] font-sans font-bold"/></div>))}
                        <div className="pt-2 border-t border-slate-200">
                             <div className="flex gap-2 mb-2 font-sans font-bold">
                                <select value={synthType} onChange={e=>setSynthType(e.target.value)} className="w-24 text-[10px] p-1.5 rounded border border-slate-200 font-sans font-bold"><option value="sawtooth">톱니파</option><option value="sine">사인파</option><option value="square">사각파</option><option value="triangle">삼각파</option><option value="noise">노이즈</option></select>
                             ...
                             <div className="flex justify-between items-center text-xs text-slate-500 font-black mt-2 font-sans font-bold"><span>반복 횟수</span><input type="number" min="1" step="1" value={loopCount} onChange={e => setLoopCount(parseInt(e.target.value) || 1)} className="w-12 border rounded px-1 text-center font-sans font-bold"/></div>
                        </div>
                    </div>
                </div>
            </div>
            <div className="h-48 bg-white/40 rounded-3xl border border-slate-300 p-3 flex flex-col gap-2 shadow-inner relative overflow-hidden font-sans font-bold">
                ...
                <div className="flex-1 rounded-2xl border border-slate-200 relative overflow-hidden z-10 bg-transparent font-sans font-bold" onContextMenu={e=>e.preventDefault()}>
