...
export const SimulatorTab = ({ audioContext, files, onAddToRack }) => {
    ...
    const renderPreview = useCallback(async () => {
        ...
        const offline = new OfflineAudioContext(1, Math.floor(sr * BASE_DURATION), sr);
        const sNode = offline.createOscillator();
        sNode.type = synthType === 'noise' ? 'sawtooth' : synthType;
        ...
        for(let i=0; i<=20; i++) {
            const t = i/20; const time = t * BASE_DURATION;
            const getV = (pts) => { const idx = pts.findIndex(p=>p.t>=t); if(idx<=0) return pts[0].v; const p1=pts[idx-1], p2=pts[idx]; return p1.v + (p2.v-p1.v)*((t-p1.t)/(p2.t-p1.t)); };
            f1.frequency.linearRampToValueAtTime(Math.max(50, 200 + (1-getV(getPts('tongueY')))*600), time);
        }
        sNode.connect(f1); f1.connect(offline.destination); sNode.start(0);
        const rendered = await offline.startRendering();
        drawWaveform(rendered);
    }, [audioContext, advTracks, synthType]);
    ...
    const drawWaveform = (buffer) => {
        ...
        ctx.clearRect(0,0,w,h); ctx.lineWidth=3; ctx.strokeStyle='#3b82f6'; ctx.beginPath();
        for(let i=0; i<w; i++) {
            let min=1, max=-1;
            for(let j=0; j<step; j++) { const d = data[i*step+j]; if(d<min) min=d; if(d>max) max=d; }
            ctx.moveTo(i, h/2 + min*(h/3)); ctx.lineTo(i, h/2 + max*(h/3));
        } ctx.stroke();
    };

    useEffect(() => { renderPreview(); }, [advTracks, renderPreview]);
    ...
    useEffect(() => {
        ...
        ctx.clearRect(0, 0, w, h); ctx.fillStyle = 'rgba(255,255,255,0.6)'; ctx.fillRect(0, RULER_HEIGHT, w, h - RULER_HEIGHT);
    ...
    return (
        <div className="flex-1 flex flex-col p-4 gap-4 animate-in fade-in overflow-hidden font-sans bg-slate-50 font-bold" onMouseUp={() => setDragPart(null)}>
                ...
                <div className="flex-1 bg-white rounded-2xl border border-slate-300 relative overflow-hidden shadow-sm flex flex-col">
                    <div className="flex-1 relative flex items-center justify-center p-4 bg-slate-100/50">
                        <svg viewBox="0 0 400 400" className="w-full h-full max-w-[380px] drop-shadow-2xl">
                            ...
                            {/* Improved Glottis Visual */}
                            <path d={`M 150 400 L 150 ${350 + liveTract.throat * 30}`} stroke="#0ea5e9" strokeWidth="22" strokeLinecap="round" opacity="0.9"/>
                            ...
                            {/* Lip Detection Area (Green area in your photo) */}
                    ...
                    <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={async()=>{ const c = await renderOneCycle(); if(!c) return; let f = c; for(let i=1; i<loopCount; i++) f = AudioUtils.concatBuffers(audioContext, f, c); onAddToRack(f, "시뮬레이션_결과"); }} className="bg-[#a3cef0] text-[#1f1e1d] px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg active:scale-95 flex items-center gap-1 hover:bg-[#209ad6] hover:text-white transition-all"><LogIn size={16} /> 보관함에 저장</button>
                        </div>
                    </div>
                </div>
            ...
            <div className="h-48 bg-white/40 rounded-3xl border border-slate-300 p-3 flex flex-col gap-2 shadow-inner relative overflow-hidden font-sans font-bold">
                {/* STATIC ENTIRE WAVEFORM BACKGROUND (Req: Photo Visual) */}
