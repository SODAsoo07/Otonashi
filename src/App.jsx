...
const HelpModal = ({ onClose }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in" onClick={onClose}>
      <div className="bg-white w-[800px] max-h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden font-sans" onClick={e => e.stopPropagation()}>
         <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
           <div className="flex items-center gap-2"><Activity className="text-[#209ad6] w-5 h-5"/><h2 className="text-lg font-black text-slate-800 tracking-tight">OTONASHI 가이드</h2></div>
           <button onClick={onClose} className="p-1 hover:bg-slate-200 rounded-lg text-slate-500 transition-colors"><X size={20}/></button>
         </div>
         <div className="p-8 overflow-y-auto custom-scrollbar text-slate-600 leading-relaxed text-sm space-y-8 font-sans">
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30 backdrop-blur-sm animate-in zoom-in-95 font-sans" onClick={onClose}>
            <div className="bg-white p-6 rounded-xl shadow-2xl w-80" onClick={e=>e.stopPropagation()}>
                <h3 className="text-lg font-black text-slate-700 mb-4 flex items-center gap-2 font-bold">{type === 'in' ? <SignalLow size={20}/> : <SignalHigh size={20}/>} Fade 설정</h3>
                <div className="flex gap-2 mb-6">
                    <button onClick={()=>setShape('linear')} className={`flex-1 py-3 rounded-lg border font-bold text-xs ${shape==='linear'?'bg-[#209ad6] text-white border-[#209ad6]':'bg-slate-50 text-slate-500 border-slate-200'}`}>직선</button>
                    <button onClick={()=>setShape('exponential')} className={`flex-1 py-3 rounded-lg border font-bold text-xs ${shape==='exponential'?'bg-[#209ad6] text-white border-[#209ad6]':'bg-slate-50 text-slate-500 border-slate-200'}`}>곡선</button>
                </div>
                <button onClick={()=>{ onApply(shape); onClose(); }} className="w-full py-3 bg-[#209ad6] text-white rounded-lg font-bold shadow-md hover:bg-[#1a85b9] transition-all">적용</button>
            </div>
        </div>
    );
};
          ...
          {files.map(f => (
            <div key={f.id} draggable onDragStart={(e) => e.dataTransfer.setData("fileId", f.id)}
              ...
              <div className="flex opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); if(window.confirm("삭제하시겠습니까?")) removeFile(f.id); }} className="p-1 hover:text-red-500"><X size={14}/></button>
              </div>
            </div>
          ))}
        </div>
...
const StudioTab = ({ audioContext, activeFile, onAddToRack, setActiveFileId, onEdit, onUndo, onRedo }) => {
    ...
    useEffect(() => {
        const handleKeyDown = (e) => { if (e.code === 'Space' && e.target.tagName !== 'INPUT') { e.preventDefault(); handlePlayPause(); } };
        ...
        <div className="flex-1 flex flex-col gap-4 p-4 font-sans overflow-y-auto custom-scrollbar h-full bg-slate-50" onDragOver={e=>e.preventDefault()}>
            <div className="flex-shrink-0 flex flex-col gap-4 font-sans">
                ...
                <div className="h-[500px] bg-white rounded-xl border border-slate-300 relative overflow-hidden shadow-inner group flex-shrink-0">
                    ...
                    ) : <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 gap-2 font-black uppercase"><Upload size={40}/><span>파일을 보관함에서 드래그하세요</span></div>}
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 min-h-min pb-10 font-bold">
                <div className="bg-white/40 rounded-xl border border-slate-300 p-4 flex flex-col gap-3 font-bold">
                    <h4 className="text-sm font-black text-[#209ad6] flex items-center gap-2"><Sliders size={18}/> 믹서</h4>
                ...
                <div className="bg-white/40 rounded-xl border border-slate-300 p-4 flex flex-col gap-3">
                    <h4 className="text-sm font-black text-emerald-500 flex items-center gap-2"><Activity size={18}/> 포먼트</h4>
                    {['f1', 'f2', 'f3'].map(f => (<div key={f}><div className="flex justify-between text-xs font-black text-slate-500 mb-1"><span>{f}</span><span>{formant[f]}Hz</span></div><input type="range" min="200" max={5000} value={formant[f]} onChange={e=>setFormant({...formant, [f]: Number(e.target.value)})} className="w-full h-1.5 bg-slate-300 appearance-none accent-emerald-500"/></div>))}
                </div>
                <div className="bg-white/40 rounded-xl border border-slate-300 p-4 flex flex-col gap-3">
                    <h4 className="text-sm font-black text-indigo-500 flex items-center gap-2"><SlidersHorizontal size={18}/> 밴드 EQ</h4>
                    {['low', 'mid', 'high'].map(band => (<div key={band}><div className="flex justify-between text-xs font-black text-slate-500 mb-1"><span>{band}</span><span>{eq[band]}dB</span></div><input type="range" min="-24" max="24" value={eq[band]} onChange={e=>setEq({...eq, [band]: Number(e.target.value)})} className="w-full h-1.5 bg-slate-300 appearance-none accent-indigo-500"/></div>))}
                </div>
...
const ConsonantTab = ({ audioContext, files, onAddToRack }) => {
    ...
    const mixConsonant = async () => {
        ...
        const vOff = vowelOffsetMs/1000, cOff = offsetMs/1000;
    ...
    return (
        <div className="flex-1 p-6 flex flex-col gap-6 animate-in fade-in font-sans overflow-y-auto bg-slate-50">
            <div className="bg-white rounded-3xl border border-slate-300 p-8 flex flex-col gap-6 shadow-sm font-bold">
                <div className="flex items-center gap-3 border-b border-slate-200 pb-4"><div className="p-2 bg-indigo-500 rounded-xl text-white"><Combine size={24}/></div><h2 className="text-xl font-black text-slate-800 tracking-tight">자음-모음 합성기</h2></div>
                <div className="flex gap-2"><button onClick={()=>setEditMode('placement')} className={`flex-1 py-2.5 text-xs rounded-lg border transition-all ${editMode==='placement'?'bg-indigo-500 text-white shadow-md':'bg-white text-slate-500'}`}>위치 / 길이</button><button onClick={()=>setEditMode('vVol')} className={`flex-1 py-2.5 text-xs rounded-lg border transition-all ${editMode==='vVol'?'bg-blue-500 text-white shadow-md':'bg-white text-slate-500'}`}>모음 볼륨</button><button onClick={()=>setEditMode('cVol')} className={`flex-1 py-2.5 text-xs rounded-lg border transition-all ${editMode==='cVol'?'bg-orange-500 text-white shadow-md':'bg-white text-slate-500'}`}>자음 볼륨</button></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 space-y-3 font-bold">
                ...
                <div className="bg-white border border-slate-300 p-4 rounded-2xl shadow-inner space-y-4">
                    <div className="flex justify-between items-center font-bold">
                        <div className="flex gap-4 text-xs font-bold"><span>자음 오프셋: {Math.round(offsetMs)}ms</span><span>모음 오프셋: {Math.round(vowelOffsetMs)}ms</span><span>자음 스트레치: {Math.round(consonantStretch*100)}%</span></div>
                        <div className="flex gap-2">
                             <button onClick={async () => { if(sourceRef.current) sourceRef.current.stop(); const b = await mixConsonant(); if(b) { const s = audioContext.createBufferSource(); s.buffer = b; s.connect(audioContext.destination); s.start(); sourceRef.current = s; setIsPlaying(true); s.onended = () => setIsPlaying(false); } }} className="px-6 py-2 bg-indigo-600 text-white rounded-xl font-bold shadow-lg">{isPlaying ? '중지' : '미리보기'}</button>
                             <button onClick={async () => { const b = await mixConsonant(); if(b) onAddToRack(b, "합성_결과"); }} className="px-6 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold border border-slate-300">저장</button>
                        </div>
                    </div>
                        ...
                        <canvas ref={canvasRef} width={1000} height={192} className="w-full h-full block cursor-crosshair" 
                            onMouseDown={(e)=> {
                                ...
                                if(editMode==='placement') setDragging('cOff');
                                     ...
                                     if(hitIdx===-1) { const t = Math.max(0, Math.min(1, (x - (vowelOffsetMs/2)) / width)); const v = Math.max(0, Math.min(1, 1-(y/192))); setVVolumePts(p => [...p, {t,v}].sort((a,b)=>a.t-b.t)); } 
                                     else setDragging(`vPoint:${hitIdx}`); 
                                } else if(editMode==='cVol') {
                                     const cBuf = files.find(f => f.id === consonantId)?.buffer; if(!cBuf) return;
                                     const width = (cBuf.duration * consonantStretch / 2.0) * 1000;
                                     const hitIdx = cVolumePts.findIndex(p=>Math.hypot(x-(offsetMs/(2000/1000)+p.t*width), y-(192-p.v*192))<15); 
                                     if(hitIdx===-1) { const t = Math.max(0, Math.min(1, (x - (offsetMs/2)) / width)); const v = Math.max(0, Math.min(1, 1-(y/192))); setCVolumePts(p => [...p, {t,v}].sort((a,b)=>a.t-b.t)); } 
                                     else setDragging(`cPoint:${hitIdx}`);
                                }
                            }}
                            onMouseMove={(e)=> {
                                if(!dragging) return; const rect = canvasRef.current.getBoundingClientRect(); const x = (e.clientX-rect.left)*(1000/rect.width); const y = (e.clientY-rect.top)*(192/rect.height);
                                if(dragging === 'cOff') setOffsetMs(p => p + e.movementX*4);
                                else if(dragging.startsWith('vPoint')) {
                                     const idx = parseInt(dragging.split(':')[1]); const vBuf = files.find(f => f.id === vowelId)?.buffer; if(!vBuf) return;
                                     const width = (vBuf.duration / 2.0) * 1000;
                                     setVVolumePts(prev => { const n = [...prev]; n[idx] = { t: Math.max(0, Math.min(1, (x - (vowelOffsetMs/2)) / width)), v: Math.max(0, Math.min(1, 1-(y/192))) }; return n.sort((a,b)=>a.t-b.t); });
                                } else if(dragging.startsWith('cPoint')) {
                                     const idx = parseInt(dragging.split(':')[1]); const cBuf = files.find(f => f.id === consonantId)?.buffer; if(!cBuf) return;
                                     const width = (cBuf.duration * consonantStretch / 2.0) * 1000;
                                     setCVolumePts(prev => { const n = [...prev]; n[idx] = { t: Math.max(0, Math.min(1, (x - (offsetMs/2)) / width)), v: Math.max(0, Math.min(1, 1-(y/192))) }; return n.sort((a,b)=>a.t-b.t); });
                                }
                            }}
                            ...
                            onContextMenu={(e)=>{ 
                                ...
                                if(editMode==='vVol') {
                                     ...
                                     const width = (vBuf.duration / 2.0) * 1000;
                                     const hitIdx = vVolumePts.findIndex(p=>Math.hypot(x-(vowelOffsetMs/(2000/1000)+p.t*width), y-(192-p.v*192))<15);
                                     if(hitIdx!==-1 && vVolumePts.length > 2) setVVolumePts(p => p.filter((_,i)=>i!==hitIdx));
                                } else if(editMode==='cVol') {
                                     const cBuf = files.find(f => f.id === consonantId)?.buffer; if(!cBuf) return;
                                     const width = (cBuf.duration * consonantStretch / 2.0) * 1000;
                                     const hitIdx = cVolumePts.findIndex(p=>Math.hypot(x-(offsetMs/(2000/1000)+p.t*width), y-(192-p.v*192))<15);
                                     if(hitIdx!==-1 && cVolumePts.length > 2) setCVolumePts(p => p.filter((_,i)=>i!==hitIdx));
                                }
                            }}
                        />
                    </div>
    ...
    return (
        <div className="flex-1 flex flex-col p-4 gap-4 animate-in fade-in overflow-hidden font-sans bg-slate-50" onMouseUp={() => { setDragPart(null); }}>
                ...
                <div className="w-72 bg-white/40 rounded-2xl border border-slate-300 p-3 flex flex-col gap-4 overflow-y-auto custom-scrollbar font-bold">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-1 mb-2">
                            ...
                            <input type="range" min="0" max="2" step="0.01" value={liveTract.volume} onChange={e => { setManualPose(true); setLiveTract(prev => ({ ...prev, volume: Number(e.target.value) })); }} className="w-full h-1.5 bg-slate-300 rounded-full accent-emerald-500" />
                        </div>
                        {[{id:'lips',label:'입술 열기'},{id:'lipLen',label:'입술 길이'},{id:'throat',label:'목 조임'},{id:'nasal',label:'비성'}].map(p => (
                            <div key={p.id} className="space-y-1">
                                ...
                                <input type="range" min="0" max="1" step="0.01" value={liveTract[p.id]} onChange={e=>{setManualPose(true); setLiveTract(prev=>({...prev,[p.id]:Number(e.target.value)}));}} className="w-full h-1 bg-slate-300 rounded-full accent-[#209ad6]" />
                            </div>
                        ))}
                        <div className="pt-2 border-t border-slate-200">
                             ...
                             <div className="flex justify-between items-center text-xs text-slate-500 font-black mt-2"><span>반복 횟수</span><input type="number" min="1" step="1" value={loopCount} onChange={e => setLoopCount(parseInt(e.target.value) || 1)} className="w-12 border rounded px-1 text-center" /></div>
                        </div>
                    </div>
                </div>
            </div>
            <div className="h-48 bg-white/40 rounded-3xl border border-slate-300 p-3 flex flex-col gap-2 shadow-inner relative overflow-hidden">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar z-10 font-black">
    ...
    return (
        <div className="h-screen w-full bg-[#f8f8f6] text-[#1f1e1d] flex flex-col font-sans overflow-hidden">
            ...
            <header className="h-14 border-b border-slate-300 bg-white flex items-center justify-between px-6 shrink-0 z-10 shadow-sm font-sans">
                ...
                <nav className="flex bg-slate-100 p-1.5 rounded-xl border border-slate-200 font-bold">
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-y-auto relative shadow-inner custom-scrollbar h-full">
