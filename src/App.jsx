    ...
    <aside className="w-64 bg-white/40 border-r border-slate-300 flex flex-col shrink-0 font-sans z-20">
      <div className="p-4 border-b border-slate-300 flex justify-between items-center bg-slate-200/50">
        ...
        <label className="cursor-pointer hover:bg-slate-300 p-1 rounded transition text-[#209ad6]" title="파일 추가">
          <Plus className="w-5 h-5"/><input type="file" multiple accept=".wav,.mp3,audio/*" className="hidden" onChange={handleFileUpload}/>
        </label>
      </div>
            ...
            <div className="flex-[3] flex flex-col gap-4 min-h-0">
                <div className="bg-white/50 rounded-xl border border-slate-300 p-2 flex justify-between items-center shadow-sm">
                    <div className="flex gap-1">
                        <button onClick={handleUndo} disabled={undoStack.length === 0} title="실행 취소" className="p-2 hover:bg-slate-200 rounded text-slate-600 disabled:opacity-30"><Undo2 size={18}/></button>
                        <div className="w-px h-6 bg-slate-300 mx-1"></div>
                        <button onClick={() => { if(!studioBuffer) return; pushUndo(); setClipboard(AudioUtils.createBufferFromSlice(audioContext, studioBuffer, editTrim.start, editTrim.end)); updateStudioBuffer(AudioUtils.deleteRange(audioContext, studioBuffer, editTrim.start, editTrim.end)); }} title="잘라내기(삭제)" className="p-2 hover:bg-slate-200 rounded text-slate-600"><Scissors size={18}/></button>
                        <button onClick={() => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(AudioUtils.createBufferFromSlice(audioContext, studioBuffer, editTrim.start, editTrim.end)); }} title="크롭(선택 영역만 남기기)" className="p-2 hover:bg-slate-200 rounded text-slate-600"><Crop size={18}/></button>
                        <button onClick={() => { if(!studioBuffer) return; setClipboard(AudioUtils.createBufferFromSlice(audioContext, studioBuffer, editTrim.start, editTrim.end)); }} title="복사" className="p-2 hover:bg-slate-200 rounded text-slate-600"><Copy size={18}/></button>
                        <div className="w-px h-6 bg-slate-300 mx-1"></div>
                        <button onClick={() => { if(!clipboard || !studioBuffer) return; pushUndo(); updateStudioBuffer(AudioUtils.insertBuffer(audioContext, studioBuffer, clipboard, editTrim.end)); }} title="붙여넣기 (삽입)" className="p-2 hover:bg-slate-200 rounded text-slate-600"><Clipboard size={18}/></button>
                        <button onClick={() => { if(!clipboard || !studioBuffer) return; pushUndo(); updateStudioBuffer(AudioUtils.mixBuffers(audioContext, studioBuffer, clipboard, editTrim.start)); }} title="오버레이 (믹스)" className="p-2 hover:bg-slate-200 rounded text-indigo-500"><Layers size={18}/></button>
                        <button onClick={() => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(AudioUtils.reverseBuffer(audioContext, studioBuffer)); }} title="좌우 반전" className="p-2 hover:bg-slate-200 rounded text-purple-500"><FlipHorizontal size={18}/></button>
                        <button onClick={async () => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(await AudioUtils.applyFade(audioContext, studioBuffer, 'in', editTrim.start, editTrim.end)); }} title="페이드 인" className="p-2 hover:bg-slate-200 rounded text-emerald-500"><TrendingUp size={18}/></button>
                        <button onClick={async () => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(await AudioUtils.applyFade(audioContext, studioBuffer, 'out', editTrim.start, editTrim.end)); }} title="페이드 아웃" className="p-2 hover:bg-slate-200 rounded text-rose-500"><TrendingDown size={18}/></button>
                    </div>
                ...
                <div className="bg-white border border-slate-300 p-6 rounded-2xl shadow-inner space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={async () => { if(!audioContext) return; const b = await mixConsonant(); if(b) onAddToRack(b, "Consonant_Mix"); }} className="px-8 py-3 bg-white border border-slate-300 text-slate-600 hover:bg-slate-50 rounded-xl font-bold transition-all shadow-sm">보관함에 저장</button>
                        </div>
                    </div>
...
const AdvancedTractTab = ({ audioContext, files, onAddToRack }) => {
    ...
    const renderAdvancedAudio = useCallback(async () => {
        ...
        sNode.connect(f1); nGain.connect(f1); f1.connect(f2); f2.connect(f3); f3.connect(nasF); nasF.connect(offline.destination);
            ...
            <div className="flex-[3] flex gap-4 min-h-0 overflow-hidden">
                <div className="flex-1 bg-white/60 rounded-2xl border border-slate-300 relative overflow-hidden shadow-sm flex flex-col">
                    <div className="flex-1 relative flex items-center justify-center p-4 bg-slate-100/50">
                        ...
                        <div className="absolute inset-0" 
                            ...
                            onMouseDown={(e) => {
                                ...
                                if (Math.hypot(nx - 0.88, ny - 0.56) < 0.3) setDragPart('lips');
                                else if (nx > 0.35 && nx < 0.75 && ny > 0.5 && ny < 0.95) { setDragPart('tongue'); setLiveTract(p => ({...p, x: nx, y: 1-ny})); }
                            }} 
                        />
                    </div>
                    <div className="p-4 bg-slate-50/80 border-t border-slate-200 flex justify-between items-center backdrop-blur-md">
                        ...
                        <div className="flex gap-2">
                             <button onClick={registerKeyframe} title="현재 포즈를 타임라인에 기록" className="bg-[#209ad6] hover:bg-[#1a85b9] text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg active:scale-95 transition-all flex items-center gap-2"><CircleDot size={16}/> 키프레임 등록</button>
                             ...
                             <button onClick={async()=>{ const res = await renderAdvancedAudio(); if(res) onAddToRack(res, "시뮬레이션_결과"); }} className="bg-[#a3cef0] hover:bg-[#209ad6] hover:text-white text-[#1f1e1d] px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg active:scale-95 transition-all flex items-center gap-1"><LogIn size={16}/> 보관함 저장</button>
                        </div>
                    </div>
                </div>
                <div className="w-72 bg-white/40 rounded-2xl border border-slate-300 p-4 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
                    <h3 className="font-black text-slate-600 uppercase tracking-widest flex items-center gap-2 text-xs"><Sliders size={20} className="text-[#209ad6]"/> 파라미터</h3>
                    <div className="space-y-4">
                        <div className="flex gap-2 mb-2">
                             {['A','E','I','O','U'].map(v=><button key={v} onClick={()=>applyPreset(v)} title={`모음 ${v}`} className="flex-1 h-9 rounded-lg bg-white border border-slate-300 font-bold text-xs hover:bg-[#209ad6] hover:text-white transition-all shadow-sm">{v}</button>)}
                        </div>
                        <div className="flex gap-2 mb-4">
                             <button onClick={()=>{setAdvTracks(prev=>prev.map(t=>t.id==='pitch'?{...t,points:[{t:0,v:110},{t:1,v:110}]}:t))}} className="flex-1 py-2 bg-white border border-slate-300 rounded-lg text-blue-500 text-xs font-bold hover:bg-blue-50 shadow-sm">Male</button>
                             <button onClick={()=>{setAdvTracks(prev=>prev.map(t=>t.id==='pitch'?{...t,points:[{t:0,v:330},{t:1,v:330}]}:t))}} className="flex-1 py-2 bg-white border border-slate-300 rounded-lg text-pink-500 text-xs font-bold hover:bg-pink-50 shadow-sm">Female</button>
                        </div>
                        ...
                        <div className="space-y-1 mt-3">
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">입력 소스 / 노이즈</span>
                             <select value={tractSourceFileId} onChange={e=>setTractSourceFileId(e.target.value)} className="w-full text-xs p-2 rounded-lg border border-slate-200 focus:border-blue-400 outline-none">
                                 <option value="">기본 신디사이저 (Sawtooth)</option>
                                 {files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
                             </select>
                             <select value={noiseSourceFileId} onChange={e=>setNoiseSourceFileId(e.target.value)} className="w-full text-xs p-2 rounded-lg border border-slate-200 focus:border-blue-400 outline-none mt-1">
                        ...
                        <div className="flex justify-between items-center text-xs font-bold text-slate-500 uppercase mt-2">
                            <span>반복 시간 (s)</span>
                            <input type="number" step="0.1" value={advDuration} onChange={e=>setAdvDuration(Number(e.target.value))} className="w-16 border rounded-md px-2 py-1 focus:ring-1 ring-blue-300 outline-none"/>
                        </div>
                    </div>
                </div>
            </div>
            <div className="h-48 bg-white/40 rounded-3xl border border-slate-300 p-4 flex flex-col gap-2 shadow-inner">
        ...
        <div className="h-screen w-full bg-[#f8f8f6] text-[#1f1e1d] flex flex-col font-sans overflow-hidden">
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-hidden relative shadow-inner">
                    {activeTab === 'editor' && <StudioTab audioContext={audioContext} activeFile={files.find(f=>f.id===activeFileId)} files={files} onAddToRack={addToRack} setActiveFileId={setActiveFileId} />}
