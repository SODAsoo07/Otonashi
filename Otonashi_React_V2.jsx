...
// ==========================================
// 3. Component: Integrated Studio Tab
// ==========================================
            ...
            <div className="flex-[3] flex flex-col gap-4 min-h-0 font-sans">
                <div className="bg-white/50 rounded-xl border border-slate-300 p-2 flex justify-between items-center shadow-sm">
                    <div className="flex gap-1 font-sans">
                        ...
                        <button onClick={() => { if(!activeBuffer || !clipboard) return; pushUndo(); const pre = AudioUtils.createBufferFromSlice(audioContext, activeBuffer, 0, editTrim.end); const post = AudioUtils.createBufferFromSlice(audioContext, activeBuffer, editTrim.end, 100); onUpdateFile(AudioUtils.concatBuffers(audioContext, AudioUtils.concatBuffers(audioContext, pre, clipboard), post)); }} disabled={!clipboard} title="붙여넣기" className="p-2 hover:bg-slate-200 rounded text-slate-600 disabled:opacity-30 transition-colors"><Clipboard size={14}/></button>
                        <button onClick={() => { if(!activeBuffer || !clipboard) return; pushUndo(); onUpdateFile(AudioUtils.mixBuffers(audioContext, activeBuffer, clipboard, editTrim.start)); }} disabled={!clipboard} title="오버레이 붙여넣기" className="p-2 hover:bg-slate-200 rounded text-indigo-500 disabled:opacity-30 transition-colors"><Layers size={14}/></button>
                        <div className="w-px h-6 bg-slate-300 mx-1"></div>
                        ...
                        <button onClick={()=>setShowAutomation(!showAutomation)} className={`p-2 rounded flex gap-1 items-center transition-all ${showAutomation?'bg-yellow-100 text-yellow-700':'hover:bg-slate-200 text-slate-600'}`}><Zap size={14}/><span className="text-xs font-bold font-sans">오토메이션</span></button>
                    </div>
                    <div className="flex gap-2">
                        <button onClick={async () => { if(!activeBuffer) return; const hasSel = editTrim.end - editTrim.start > 1; const b = hasSel ? AudioUtils.createBufferFromSlice(audioContext, activeBuffer, editTrim.start, editTrim.end) : activeBuffer; const res = await renderStudioAudio(b); onAddToRack(res, (activeFile?.name || "Studio") + "_결과"); }} className="bg-[#a3cef0] hover:bg-[#209ad6] hover:text-white text-[#1f1e1d] px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 shadow-sm transition-colors font-sans"><LogIn size={14}/> 보관함에 저장</button>
                    </div>
                </div>
                <div className="flex-1 bg-white rounded-xl border border-slate-300 relative overflow-hidden shadow-inner font-sans">
                    {activeBuffer ? <canvas ref={canvasRef} width={1000} height={400} className="w-full h-full object-fill cursor-crosshair" 
                        onMouseDown={e=>{ const rect=e.currentTarget.getBoundingClientRect(); if(showAutomation) { const x=e.clientX-rect.left; const y=e.clientY-rect.top; const t=x/rect.width; const v=2-(y/rect.height*2); setVolumeKeyframes(prev=>[...prev,{t:Math.max(0,Math.min(1,t)), v:Math.max(0,v)}].sort((a,b)=>a.t-b.t)); } else { const p=(e.clientX-rect.left)/rect.width*100; if(Math.abs(p-editTrim.start)<2) setDragTarget('start'); else if(Math.abs(p-editTrim.end)<2) setDragTarget('end'); else setDragTarget('new'); if(dragTarget==='new') setEditTrim({start:p, end:p}); } }}
                ...
                <div className="bg-white/40 rounded-xl border border-slate-300 p-3 flex flex-col gap-3">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="pt-1 border-t border-slate-200">
                             <div className="flex justify-between text-[9px] font-bold text-slate-500 mb-1 uppercase font-sans"><span>공명 (Resonance)</span><span>{formant.resonance.toFixed(1)}</span></div>
        ...
        <div className="flex-1 p-6 flex flex-col gap-6 animate-in fade-in overflow-hidden font-sans">
            <div className="bg-white/60 rounded-3xl border border-slate-300 p-8 flex flex-col gap-6 shadow-sm font-sans">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-sans">
                    ...
                    <div className="space-y-4 bg-slate-50/50 p-6 rounded-2xl border border-slate-300 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
...
const AdvancedTractTab = ({ audioContext, files, onAddToRack }) => {
    ...
    const handlePlayPauseSim = async () => {
        ...
        const animate = () => {
            ...
            setPlayHeadPos(current / res.duration);
    ...
    useEffect(() => { 
        ...
        if(isAdvPlaying || isPaused) {
           ...
           cancelAnimationFrame(animRef.current); setIsAdvPlaying(false); setIsPaused(false);
        }
    ...
    const handleTimelineMouseDown = useCallback((e) => {
        ...
        if (my < RULER_HEIGHT) { 
            ...
            if(isAdvPlaying) { if(simPlaySourceRef.current) try { simPlaySourceRef.current.stop(); } catch(e) {} cancelAnimationFrame(animRef.current); setIsAdvPlaying(false); setIsPaused(true); }
    ...
    return (
        <div className="flex-1 flex flex-col p-4 gap-4 animate-in fade-in overflow-hidden font-sans" onMouseUp={() => setDraggingKeyframe(null)}>
            <div className="flex-[3] flex gap-4 min-h-0 overflow-hidden font-sans font-sans font-sans">
                <div className="flex-1 bg-white/60 rounded-2xl border border-slate-300 flex flex-col relative overflow-hidden shadow-sm">
                    <div className="flex-1 relative flex items-center justify-center p-3 min-h-0">
                        ...
                        <div className="absolute inset-0 flex z-20 font-sans">
                            <div className="flex-1 cursor-crosshair font-sans" onMouseDown={(e) => { const rect = e.currentTarget.getBoundingClientRect(); const update = (ce) => { const x = Math.max(0, Math.min(1, (ce.clientX - rect.left) / rect.width)); const y = Math.max(0, Math.min(1, 1 - (ce.clientY - rect.top) / rect.height)); setLiveTract(prev => { const n = { ...prev, x, y }; updateLiveAudio(n.x, n.y, n.lips, n.throat, n.lipLen, n.nasal); return n; }); }; update(e); startLivePreview(); const mv = (me) => update(me); const up = () => { window.removeEventListener('mousemove', mv); window.removeEventListener('mouseup', up); stopLivePreview(); }; window.addEventListener('mousemove', mv); window.addEventListener('mouseup', up); }}></div>
                            <div className="w-36 bg-slate-400/5 hover:bg-slate-400/10 transition-colors cursor-move border-l border-slate-200 font-sans" onMouseDown={(e) => { const rect = e.currentTarget.getBoundingClientRect(); const update = (ce) => { const lipLen = Math.max(0, Math.min(1, (ce.clientX - rect.left) / rect.width)); const lips = Math.max(0, Math.min(1, 1 - (ce.clientY - rect.top) / rect.height)); setLiveTract(prev => { const n = { ...prev, lips, lipLen }; updateLiveAudio(n.x, n.y, n.lips, n.throat, n.lipLen, n.nasal); return n; }); }; update(e); startLivePreview(); const mv = (me) => update(me); const up = () => { window.removeEventListener('mousemove', mv); window.removeEventListener('mouseup', up); stopLivePreview(); }; window.addEventListener('mousemove', mv); window.addEventListener('mouseup', up); }}>
                                <div className="h-full w-full flex flex-col items-center justify-center opacity-30 pointer-events-none text-center p-2 font-sans font-sans font-sans font-sans"><MoveHorizontal className="w-5 h-5 mb-1"/><span className="text-[8px] font-black uppercase tracking-widest font-sans font-sans font-sans font-sans">입술 컨트롤</span></div>
                            </div>
                        </div>
                    </div>
                    <div className="p-2 px-4 bg-white/80 border-t border-slate-200 flex justify-between items-center backdrop-blur-md shrink-0 font-sans font-sans">
                        <button onClick={()=>{const t=playHeadPos;setAdvTracks(prev=>prev.map(track=>{if(track.group!=='adj')return track;let val=null;if(track.id==='tongueX')val=liveTract.x;else if(track.id==='tongueY')val=liveTract.y;else if(track.id==='lips')val=liveTract.lips;else if(track.id==='lipLen')val=liveTract.lipLen;else if(track.id==='throat')val=liveTract.throat;else if(track.id==='nasal')val=liveTract.nasal;if(val===null)return track;const other=track.points.filter(p=>Math.abs(p.t-t)>0.005);return{...track,points:[...other,{t,v:val}].sort((a,b)=>a.t-b.t)};}));}} className="bg-[#209ad6] hover:bg-[#1a85b9] text-white px-4 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1.5 active:scale-95 transition-all font-sans font-sans font-sans font-sans"><CircleDot className="w-3.5 h-3.5"/> 기록</button>
                        <div className="flex gap-1.5 font-sans font-sans">
                            <button onClick={handlePlayPauseSim} className="bg-[#209ad6] hover:bg-[#1a85b9] px-4 py-1.5 rounded-lg text-white font-bold text-xs flex items-center gap-1 active:scale-95 transition-all font-sans font-sans font-sans font-sans">{isAdvPlaying ? <Pause size={14} fill="currentColor"/> : <Play size={14} fill="currentColor"/>} {isAdvPlaying ? '일시정지' : '재생'}</button>
                            <button onClick={async()=>{ const res = await renderAdvancedAudio(); if(res) onAddToRack(res, "시뮬레이션_" + simIndex); setSimIndex(si => si + 1); }} className="bg-[#a3cef0] hover:bg-[#209ad6] hover:text-white text-[#1f1e1d] px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-colors font-sans font-sans font-sans font-sans">보관함에 저장</button>
                        </div>
                    </div>
                </div>
                <div className="w-72 bg-white/40 rounded-2xl border border-slate-300 p-3 flex flex-col gap-3 overflow-y-auto shrink-0 custom-scrollbar font-sans font-sans font-sans">
                    <h3 className="font-black text-slate-600 uppercase tracking-widest flex items-center gap-2 text-[10px] font-sans font-sans font-sans font-sans font-sans"><Sliders size={20} className="text-[#209ad6]"/> 설정</h3>
                    <div className="space-y-4 font-sans font-sans font-sans">
                        <div className="bg-white/50 p-2.5 rounded-xl border border-slate-200 font-sans font-sans font-sans font-sans font-sans">
                           <span className="text-[9px] text-slate-500 font-bold uppercase block mb-1 font-sans font-sans font-sans">음성 프리셋</span>
                           <div className="grid grid-cols-5 gap-1 font-sans font-sans font-sans font-sans">{['A','I','U','E','O'].map(v=><button key={v} onClick={()=>{ let tX=0.5, tY=0.5, lips=0.5, len=0.5; switch(v) { case 'A': tX=0.2; tY=0.1; lips=1.0; len=0.5; break; case 'I': tX=0.9; tY=1.0; lips=1.0; len=1.0; break; case 'U': tX=0.2; tY=0.9; lips=0.2; len=0.1; break; case 'E': tX=0.8; tY=0.6; lips=0.8; len=0.8; break; case 'O': tX=0.2; tY=0.5; lips=0.4; len=0.2; break; } setLiveTract(prev=>({...prev, x:tX, y:tY, lips, lipLen:len})); updateLiveAudio(tX, tY, lips, liveTract.throat, len, liveTract.nasal); }} className="bg-white border border-slate-200 hover:bg-[#209ad6] hover:text-white text-slate-600 h-7 rounded-lg text-[10px] font-black shadow-xs transition-colors font-sans font-sans font-sans font-sans">{v}</button>)}</div>
                        </div>

                        <div className="space-y-3 px-1 text-[10px] font-sans font-sans font-sans font-sans">
                            {[
                                ...
                                <div key={p.id} className="space-y-1 font-sans font-sans font-sans font-sans">
                                    <div className="flex justify-between font-bold text-slate-500 uppercase tracking-tighter font-sans font-sans font-sans font-sans"><span>{p.label}</span><span>{Math.round(liveTract[p.id]*100)}%</span></div>
                                    <input type="range" min="0" max="1" step="0.01" value={liveTract[p.id]} 
                                           ...
                                           className={`w-full h-1 bg-slate-300 appearance-none rounded-full font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans ${p.color}`}/>
                                </div>
                            ))}
                            <div className="h-px bg-slate-200 my-1 font-sans font-sans font-sans font-sans font-sans"></div>
                            <div className="space-y-1 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><span className="text-[9px] text-slate-500 font-bold uppercase font-sans font-sans font-sans font-sans">음원 소스 (Base)</span><select value={tractSourceType} onChange={e=>setTractSourceType(e.target.value)} className="w-full bg-white border border-slate-200 rounded p-1 outline-none font-bold text-[10px] font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><option value="synth">기본 신디사이저</option><option value="file">보관함 파일</option></select></div>
                            {tractSourceType==='file' && <select value={tractSourceFileId} onChange={e=>setTractSourceFileId(e.target.value)} className="w-full bg-white border border-slate-200 rounded p-1 text-[9px] mt-1 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">{files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select>}
                            
                            <div className="space-y-1 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><span className="text-[9px] text-slate-500 font-bold uppercase font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">노이즈 소스 (Noise)</span><select value={larynxParams.noiseSourceType} onChange={e=>setLarynxParams({...larynxParams, noiseSourceType:e.target.value})} className="w-full bg-white border border-slate-200 rounded p-1 outline-none font-bold text-[10px] font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><option value="generated">기본 화이트 노이즈</option><option value="file">보관함 파일</option></select></div>
                            {larynxParams.noiseSourceType==='file' && <select value={larynxParams.noiseSourceFileId} onChange={e=>setLarynxParams({...larynxParams, noiseSourceFileId:e.target.value})} className="w-full bg-white border border-slate-200 rounded p-1 text-[9px] mt-1 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">{files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select>}

                            <div className="flex justify-between items-center pt-2 font-bold text-slate-600 uppercase font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><span>반복 재생</span><input type="checkbox" checked={larynxParams.loopOn} onChange={e=>setLarynxParams(prev=>({...prev, loopOn:e.target.checked}))} className="w-3.5 h-3.5 rounded text-[#209ad6] border-slate-300 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"/></div>
                            <div className="pt-2 font-bold text-slate-600 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><div className="flex justify-between mb-1.5 uppercase font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><span>총 길이</span><span>{advDuration.toFixed(1)}s</span></div><input type="range" min="0.5" max="5" step="0.1" value={advDuration} onChange={e=>setAdvDuration(Number(e.target.value))} className="w-full h-1 bg-slate-300 appearance-none accent-[#209ad6] rounded-full font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"/></div>
                        </div>
                    </div>
                </div>
            </div>
            <div className="h-4 bg-slate-200/50 rounded-full mx-2 relative overflow-hidden shrink-0 border border-slate-300 font-sans">
            ...
            <div className="h-48 flex flex-col gap-2 bg-white/40 rounded-2xl border border-slate-300 p-2 shadow-sm relative overflow-hidden shrink-0 font-sans font-sans font-sans">
                <div className="flex items-center justify-between gap-2 pb-0.5 mb-0.5 px-1 font-sans font-sans font-sans">
                    <div className="flex gap-1 overflow-x-auto font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                        {advTracks.map(t=><button key={t.id} onClick={()=>setSelectedTrackId(t.id)} className={`px-2.5 py-1 text-[9px] font-black border rounded-full transition whitespace-nowrap shadow-xs font-sans font-sans font-sans font-sans font-sans font-sans font-sans ${selectedTrackId===t.id?'bg-[#209ad6] text-white border-[#209ad6]':'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>{t.name}</button>)}
                    </div>
                    <div className="flex gap-1 font-sans font-sans font-sans font-sans font-sans">
                        <button onClick={()=>{ setAdvTracks(prev => prev.map(t => t.id === selectedTrackId ? { ...t, points: [{t:0, v:t.id === 'pitch' ? 220 : (t.id === 'gain' ? 1 : (t.id === 'breath' ? 0.01 : 0.5))}, {t:1, v:t.id === 'pitch' ? 220 : (t.id === 'gain' ? 1 : (t.id === 'breath' ? 0.01 : 0.5))}] } : t)); }} title="항목 초기화" className="p-1 rounded bg-white border border-slate-200 text-slate-400 hover:text-orange-500 transition-colors font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><RotateCcw size={12}/></button>
                        <button onClick={()=>{ setAdvTracks(prev => prev.map(t => ({ ...t, points: [{t:0, v:t.id === 'pitch' ? 220 : (t.id === 'gain' ? 1 : (t.id === 'breath' ? 0.01 : 0.5))}, {t:1, v:t.id === 'pitch' ? 220 : (t.id === 'gain' ? 1 : (t.id === 'breath' ? 0.01 : 0.5))}] }))); }} title="전체 초기화" className="p-1 rounded bg-white border border-slate-200 text-slate-400 hover:text-red-500 transition-colors font-bold uppercase font-sans font-sans font-sans font-sans font-sans"><RotateCcw size={12} className="stroke-[3] font-sans font-sans font-sans font-sans font-sans"/></button>
                        <button onClick={()=>setClickToAdd(!clickToAdd)} title="클릭 키프레임 토글" className={`p-1 rounded-lg border transition-all shadow-sm shrink-0 ${clickToAdd ? 'bg-[#209ad6] text-white border-[#209ad6]' : 'bg-white text-slate-400 border-slate-200 hover:text-slate-600'} font-sans font-sans font-sans font-sans font-sans font-sans font-sans`}><MousePointer2 size={14}/></button>
                    </div>
                </div>
                <div className="flex-1 bg-white rounded-xl border border-slate-300 relative overflow-hidden shadow-inner font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                    <canvas ref={canvasRef} width={1000} height={200} className="w-full h-full block cursor-crosshair font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans" onMouseDown={handleTimelineMouseDown} 
                        onMouseMove={(e)=>{
                            if(!canvasRef.current) return;
                            ...
                            if(!draggingKeyframe) { const track=advTracks.find(t=>t.id===selectedTrackId); const hitIdx=track.points.findIndex(p=>Math.hypot((p.t*rect.width)-mx, (RULER_HEIGHT + (1 - (p.v - track.min) / (track.max - track.min)) * (rect.height-RULER_HEIGHT))-my) < 15); setHoveredKeyframe(hitIdx!==-1?{trackId:selectedTrackId,index:hitIdx}:null); } 
                            else { if(draggingKeyframe.isPlayhead){setPlayHeadPos(t);}else{ const gH=rect.height-RULER_HEIGHT; const nV=Math.max(0,Math.min(1,1-((my-RULER_HEIGHT)/gH))); const track=advTracks.find(tr=>tr.id===draggingKeyframe.trackId); const val=track.min+(nV*(track.max-track.min)); setAdvTracks(prev=>prev.map(tr=>tr.id===draggingKeyframe.trackId?{...tr,points:tr.points.map((p,i)=>i===draggingKeyframe.index?{t,v:val}:p)}:tr)); } }
