...
import { 
  ...
  CloudUpload, DownloadCloud, UploadCloud, FlipHorizontal, ArrowLeftRight, Crop, FilePlus
} from 'lucide-react';
...
// ==========================================
// 2. Component: File Rack (Left Sidebar)
// ==========================================
  ...
  return (
    <aside className="w-64 bg-white/40 border-r border-slate-300 flex flex-col shrink-0 font-sans z-20">
      <div className="p-4 border-b border-slate-300 flex justify-between items-center bg-slate-200/50">
        <span className="text-sm font-bold text-slate-600 uppercase tracking-wider flex items-center gap-2">파일 보관함</span>
      ...
      <div className="p-3 text-xs text-slate-400 font-bold text-center border-t border-slate-200/50">
...
const StudioTab = ({ audioContext, activeFile, files, onUpdateFile, onAddToRack, setActiveFileId }) => {
    ...
    const handlePlayPause = async () => {
        if(isPlaying) {
            if (sourceRef.current) { try { sourceRef.current.stop(); } catch(e) {} pauseOffsetRef.current = audioContext.currentTime - startTimeRef.current; setIsPlaying(false); setIsPaused(true); }
            return;
        }
        ...
        >
            <div className="flex-[3] flex flex-col gap-4 min-h-0">
                <div className="bg-white/50 rounded-xl border border-slate-300 p-2 flex justify-between items-center shadow-sm">
                    <div className="flex gap-1">
                        ...
                        <button onClick={() => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(AudioUtils.createBufferFromSlice(audioContext, studioBuffer, editTrim.start, editTrim.end)); }} title="크롭(선택 영역만 남기기)" className="p-2 hover:bg-slate-200 rounded text-slate-600"><Crop size={16}/></button>
                        ...
                        <button onClick={() => { if(!clipboard || !studioBuffer) return; pushUndo(); updateStudioBuffer(AudioUtils.mixBuffers(audioContext, studioBuffer, clipboard, editTrim.start)); }} title="오버레이 (믹스)" className="p-2 hover:bg-slate-200 rounded text-indigo-500"><Layers size={16}/></button>
                        ...
                        <button onClick={async () => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(await AudioUtils.applyFade(audioContext, studioBuffer, 'in', editTrim.start, editTrim.end)); }} title="페이드 인" className="p-2 hover:bg-slate-200 rounded text-emerald-500"><TrendingUp size={16}/></button>
                        <button onClick={async () => { if(!studioBuffer) return; pushUndo(); updateStudioBuffer(await AudioUtils.applyFade(audioContext, studioBuffer, 'out', editTrim.start, editTrim.end)); }} title="페이드 아웃" className="p-2 hover:bg-slate-200 rounded text-rose-500"><TrendingDown size={16}/></button>
                    </div>
                    <div className="flex gap-2">
                        <button onClick={() => { if(!studioBuffer) return; const sel = AudioUtils.createBufferFromSlice(audioContext, studioBuffer, editTrim.start, editTrim.end); if(sel) onAddToRack(sel, (activeFile?.name || "Extract") + "_cut"); }} title="선택 영역 새 파일로 저장" className="bg-white text-slate-700 border border-slate-300 px-3 py-1.5 rounded text-sm font-bold flex items-center gap-1 hover:bg-slate-100 transition-colors shadow-sm"><FilePlus size={16}/></button>
                        <button onClick={async () => { if(!studioBuffer || !audioContext) return; const res = await renderStudioAudio(studioBuffer); if(res) onAddToRack(res, (activeFile?.name || "Studio") + "_결과"); }} title="결과물 저장" className="bg-[#a3cef0] text-[#1f1e1d] px-3 py-1.5 rounded text-sm font-bold flex items-center gap-1 hover:bg-[#209ad6] hover:text-white transition-colors shadow-sm"><LogIn size={16}/> 보관함에 저장</button>
                    </div>
                </div>
                <div className="flex-1 bg-white rounded-xl border border-slate-300 relative overflow-hidden shadow-inner">
                    ...
                    /> : <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 opacity-30 gap-2 font-bold uppercase pointer-events-none"><Upload size={32}/> 파일을 여기에 드래그하세요</div>}
                </div>
            </div>
            <div className="flex-[2] grid grid-cols-1 md:grid-cols-4 gap-4 min-h-0 overflow-y-auto custom-scrollbar font-sans">
                <div className="bg-white/40 rounded-xl border border-slate-300 p-3 flex flex-col gap-3">
                    <h4 className="text-xs font-black text-[#209ad6] uppercase tracking-widest flex items-center gap-2 font-sans"><Sliders size={16}/> 믹서</h4>
                    <div className="space-y-1">
                        <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase"><span>마스터 볼륨</span><span>{Math.round(masterGain*100)}%</span></div>
                        ...
                        <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase mt-2"><span>피치 (Cents)</span><span>{pitchCents}</span></div>
                        <input type="range" min="-1200" max="1200" step="10" value={pitchCents} onChange={e=>setPitchCents(Number(e.target.value))} className="w-full h-1 bg-slate-300 appearance-none accent-[#209ad6]"/>
                        <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase mt-2"><span>젠더 시프트</span><span>{genderShift.toFixed(2)}x</span></div>
                        <input type="range" min="0.5" max="2.0" step="0.05" value={genderShift} onChange={e=>setGenderShift(Number(e.target.value))} className="w-full h-1 bg-slate-300 appearance-none accent-pink-500"/>
                    </div>
                </div>
                <div className="bg-white/40 rounded-xl border border-slate-300 p-3 flex flex-col gap-3">
                    <h4 className="text-xs font-black text-emerald-500 uppercase tracking-widest flex items-center gap-2 font-sans"><Activity size={16}/> 포먼트 공명</h4>
                    {['f1', 'f2', 'f3'].map(f => (
                        <div key={f}>
                            <div className="flex justify-between text-[10px] font-bold text-slate-500 mb-1 uppercase"><span>{f} (Hz)</span><span>{formant[f]}</span></div>
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase mt-1"><span>공명 (Q)</span><span>{formant.resonance.toFixed(1)}</span></div>
                    <input type="range" min="1" max="20" step="0.1" value={formant.resonance} onChange={e=>setFormant({...formant, resonance: Number(e.target.value)})} className="w-full h-1 bg-slate-300 appearance-none accent-pink-400"/>
                </div>
                <div className="bg-white/40 rounded-xl border border-slate-300 p-3 flex flex-col gap-3">
                    <h4 className="text-xs font-black text-indigo-500 uppercase tracking-widest flex items-center gap-2 font-sans"><SlidersHorizontal size={16}/> 밴드 EQ</h4>
                    {['low', 'mid', 'high'].map(band => (
                        <div key={band}>
                            <div className="flex justify-between text-[10px] font-bold text-slate-500 mb-1 uppercase"><span>{band}</span><span>{eq[band]}dB</span></div>
                            <input type="range" min="-24" max="24" value={eq[band]} onChange={e=>setEq({...eq, [band]: Number(e.target.value)})} className="w-full h-1 bg-slate-300 appearance-none accent-indigo-500"/>
                        </div>
                    ))}
                </div>
                <div className="bg-white/40 rounded-xl border border-slate-300 p-3 flex flex-col gap-3 justify-end">
                    <div className="flex gap-2">
                        <button onClick={handleStop} title="정지" className="p-2 bg-slate-200 hover:bg-slate-300 rounded-lg text-slate-600 transition-all"><Square size={16} fill="currentColor"/></button>
                        <button onClick={handlePlayPause} title={isPlaying ? '일시정지' : '재생'} className="flex-1 py-2 bg-[#209ad6] hover:bg-[#1a85b9] text-white rounded-lg font-bold text-[10px] flex items-center justify-center gap-2 shadow-sm transition-all">
                            {isPlaying ? <Pause size={16} fill="currentColor"/> : <Play size={16} fill="currentColor"/>} {isPlaying ? '중지' : '미리보기'}
                        </button>
                    </div>
                </div>
            </div>
...
const ConsonantTab = ({ audioContext, files, onAddToRack }) => {
    ...
    useEffect(() => {
        ...
        const drawWave = (buf, color, offsetY, widthScale = 1.0) => {
            ...
            const displayedDuration = buf.duration * widthScale;
            const pixelWidth = (displayedDuration / 2.0) * w; // 2s window
            const amp = h / 4;
            ctx.beginPath(); ctx.strokeStyle = color; ctx.lineWidth = 2;
            for(let i=0; i<pixelWidth; i++) {
                let min=1.0, max=-1.0;
                const startIdx = Math.floor((i/pixelWidth) * data.length);
                const endIdx = Math.floor(((i+1)/pixelWidth) * data.length);
        ...
        let vWidth = 0;
        if(vBuf) {
            vWidth = drawWave(vBuf, '#3b82f6', h/2);
            if (editMode === 'vVol') drawEnvelope(vVolumePts, '#1d4ed8', vWidth, 0);
        }

        if(cBuf) {
            const msPerPixel = 2000 / w; 
            const pixelOffset = offsetMs / msPerPixel;
            ctx.save(); ctx.translate(pixelOffset, 0);
            const drawnWidth = drawWave(cBuf, '#f97316', h/2, consonantStretch);
            if (editMode === 'placement') {
                 ctx.fillStyle = '#3b82f6'; ctx.fillRect(drawnWidth - 6, h/2 - 25, 12, 50);
            }
            ctx.restore();
            
            if (editMode === 'cVol') {
                drawEnvelope(cVolumePts, '#ea580c', drawnWidth, pixelOffset);
            }
        }
    ...
    const handleCanvasMouseDown = (e) => {
        ...
        if (editMode === 'placement') {
            ...
            const cBuf = files.find(f => f.id === consonantId)?.buffer;
            if (cBuf) {
                const displayedDuration = cBuf.duration * consonantStretch;
                const pixelWidth = (displayedDuration / 2.0) * w;
                const handleX = pixelOffset + pixelWidth;
                if (Math.abs(x * (w/rect.width) - handleX) < 20) { setDragging('stretch'); return; }
            }
            setDragging('offset');
        } else if (editMode === 'vVol') {
            const vBuf = files.find(f => f.id === vowelId)?.buffer;
            if (!vBuf) return;
            const width = (vBuf.duration / 2.0) * w;
            const hitIdx = vVolumePts.findIndex(p => {
                const px = p.t * width;
                const py = h - p.v * h;
                return Math.hypot(x*(w/rect.width) - px, y*(h/rect.height) - py) < 10;
            });
            if (hitIdx !== -1) { setDragging(`vPoint:${hitIdx}`); }
            else {
                const t = Math.max(0, Math.min(1, (x * (w/rect.width)) / width));
                const v = Math.max(0, Math.min(1, 1 - (y * (h/rect.height)) / h));
                const newPts = [...vVolumePts, {t,v}].sort((a,b)=>a.t-b.t);
                setVVolumePts(newPts);
                setDragging(`vPoint:${newPts.findIndex(p=>p.t===t)}`);
            }
             ...
             const msPerPixel = 2000 / w; 
             const pixelOffset = offsetMs / msPerPixel;
             const width = (cBuf.duration * consonantStretch / 2.0) * w;
             
             const hitIdx = cVolumePts.findIndex(p => {
                 const px = pixelOffset + p.t * width;
                 const py = h - p.v * h;
                 return Math.hypot(x*(w/rect.width) - px, y*(h/rect.height) - py) < 10;
             });
             ...
             if (hitIdx !== -1) { setDragging(`cPoint:${hitIdx}`); }
                 ...
                 const relX = (x * (w/rect.width)) - pixelOffset;
                 const t = Math.max(0, Math.min(1, relX / width));
                 const v = Math.max(0, Math.min(1, 1 - (y * (h/rect.height)) / h));
                 const newPts = [...cVolumePts, {t,v}].sort((a,b)=>a.t-b.t);
                 setCVolumePts(newPts);
                 setDragging(`cPoint:${newPts.findIndex(p=>p.t===t)}`);
             }
        }
    };
    ...
    useEffect(() => {
        const move = (e) => {
            ...
            if (dragging === 'offset') {
                ...
                setOffsetMs(prev => Math.max(-500, Math.min(1500, prev + deltaX * 4)));
            } else if (dragging === 'stretch') {
                const w = canvasRef.current.width;
                const canvasX = (e.clientX - rect.left) * (w / rect.width);
                const msPerPixel = 2000 / w; 
                const pixelOffset = offsetMs / msPerPixel;
                const newWidthPx = canvasX - pixelOffset;
                if (newWidthPx < 10) return; 
                const cBuf = files.find(f => f.id === consonantId)?.buffer;
                if (cBuf) {
                    const originalWidthPx = (cBuf.duration / 2.0) * w;
                    setConsonantStretch(newWidthPx / originalWidthPx);
                }
                ...
                const t = Math.max(0, Math.min(1, (e.clientX - rect.left) * (w/rect.width) / width));
                const v = Math.max(0, Math.min(1, 1 - (e.clientY - rect.top) * (h/rect.height) / h));
                ...
                const relX = ((e.clientX - rect.left) * (w/rect.width)) - pixelOffset;
                const t = Math.max(0, Math.min(1, relX / width));
                const v = Math.max(0, Math.min(1, 1 - (e.clientY - rect.top) * (h/rect.height) / h));
    ...
    return (
        <div className="flex-1 p-6 flex flex-col gap-6 animate-in fade-in font-sans">
            <div className="bg-white/60 rounded-3xl border border-slate-300 p-8 flex flex-col gap-6 shadow-sm">
                ...
                <div className="flex gap-2 mb-2">
                    <button onClick={()=>setEditMode('placement')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${editMode==='placement'?'bg-indigo-500 text-white':'bg-white text-slate-500 border border-slate-200'}`}>배치 / 길이 조절</button>
                    <button onClick={()=>setEditMode('vVol')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${editMode==='vVol'?'bg-blue-500 text-white':'bg-white text-slate-500 border border-slate-200'}`}>모음 볼륨 (Vowel)</button>
                    <button onClick={()=>setEditMode('cVol')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${editMode==='cVol'?'bg-orange-500 text-white':'bg-white text-slate-500 border border-slate-200'}`}>자음 볼륨 (Consonant)</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4 bg-slate-50/50 p-6 rounded-2xl border border-slate-200">
                        <div className="flex justify-between items-center">
                            <label className="text-xs font-black text-indigo-500 uppercase tracking-widest block">Vowel Volume (모음 볼륨)</label>
                            <span className="text-[10px] font-bold text-slate-500">{Math.round(vowelGain*100)}%</span>
                        </div>
                        <select value={vowelId} onChange={e=>setVowelId(e.target.value)} className="w-full p-3 rounded-xl border border-slate-300 font-bold text-sm outline-none focus:border-indigo-400">
                            <option value="">모음 트랙 선택...</option>
                            {files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
                        </select>
                        <input type="range" min="0" max="2" step="0.1" value={vowelGain} onChange={e=>setVowelGain(Number(e.target.value))} className="w-full accent-indigo-500"/>
                    </div>
                    <div className="space-y-4 bg-slate-50/50 p-6 rounded-2xl border border-slate-200">
                        <div className="flex justify-between items-center">
                            <label className="text-xs font-black text-pink-500 uppercase tracking-widest block">Consonant Volume (자음 볼륨)</label>
                            <span className="text-[10px] font-bold text-slate-500">{Math.round(consonantGain*100)}%</span>
                        </div>
                        <select value={consonantId} onChange={e=>setConsonantId(e.target.value)} className="w-full p-3 rounded-xl border border-slate-300 font-bold text-sm outline-none focus:border-pink-400">
                            <option value="">자음 트랙 선택...</option>
                            {files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
                        </select>
                        <input type="range" min="0" max="2" step="0.1" value={consonantGain} onChange={e=>setConsonantGain(Number(e.target.value))} className="w-full accent-pink-500"/>
                    </div>
                </div>
                <div className="bg-white border border-slate-300 p-6 rounded-2xl shadow-inner space-y-4">
                    <div className="flex justify-between items-center">
                        <div className="flex gap-4">
                            <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2"><MoveHorizontal size={16} className="text-slate-400"/> 오프셋: <span className="text-indigo-600 font-bold">{Math.round(offsetMs)}ms</span></h3>
                            <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2"><ArrowLeftRight size={16} className="text-slate-400"/> 길이: <span className="text-pink-600 font-bold">{Math.round(consonantStretch*100)}%</span></h3>
                        </div>
                    ...
                    <div className="relative h-40 w-full bg-slate-100 rounded-xl overflow-hidden border border-slate-200 cursor-crosshair group">
                        <canvas ref={canvasRef} width={1000} height={160} className="w-full h-full block" onMouseDown={handleCanvasMouseDown}/>
                        <div className="absolute top-2 right-2 text-[10px] font-bold text-slate-400 bg-white/80 px-2 py-1 rounded pointer-events-none border border-slate-200">
...
// ==========================================
// 5. Component: Advanced Tract Tab (Simulator)
// ==========================================
const AdvancedTractTab = ({ audioContext, files, onAddToRack }) => {
    ...
    const registerKeyframe = () => {
        ...
        setAdvTracks(prev => prev.map(tr => {
            ...
            const idx = tr.points.findIndex(p => Math.abs(p.t - t) < threshold);
            let newPoints = [...tr.points];
            if (idx !== -1) { newPoints[idx] = { ...newPoints[idx], v: val }; } 
            else { newPoints.push({ t, v: val }); newPoints.sort((a,b) => a.t - b.t); }
    ...
    const getInterpolatedValue = useCallback((trackId, t) => {
        ...
        const p2 = track.points[idx];
        return p1.v + (p2.v - p1.v) * ((t - p1.t) / (p2.t - p1.t));
    }, [advTracks]);

    useEffect(() => {
        if (manualPose || dragPart) return; 
        if (isAdvPlaying || draggingKeyframe?.isPlayhead || true) { 
    ...
    const renderAdvancedAudio = useCallback(async () => {
        ...
        const totalLen = Math.max(1, Math.floor(sr * advDuration));
        const offline = new OfflineAudioContext(1, totalLen, sr);
        
        let sNode;
        const customInput = files.find(f => f.id === tractSourceFileId)?.buffer;
        if (customInput) {
            sNode = offline.createBufferSource();
            sNode.buffer = customInput;
            sNode.loop = true;
        } else {
            sNode = offline.createOscillator();
            sNode.type = 'sawtooth';
            const tP = advTracks.find(t=>t.id==='pitch').points;
            sNode.frequency.setValueAtTime(tP[0].v, 0);
            tP.forEach(p => sNode.frequency.linearRampToValueAtTime(p.v, p.t * advDuration));
        }

        let nNode;
        const customNoise = files.find(f => f.id === noiseSourceFileId)?.buffer;
        if (customNoise) {
            nNode = offline.createBufferSource(); nNode.buffer = customNoise; nNode.loop = true;
        } else {
            nNode = offline.createBufferSource();
            const nb = offline.createBuffer(1, totalLen, sr);
            const nd = nb.getChannelData(0); for(let i=0; i<totalLen; i++) nd[i] = Math.random() * 2 - 1;
            nNode.buffer = nb;
        }
        const nGain = offline.createGain();
        const bP = advTracks.find(t=>t.id==='breath').points;
        nGain.gain.setValueAtTime(bP[0].v, 0); bP.forEach(p => nGain.gain.linearRampToValueAtTime(p.v, p.t * advDuration));
        nNode.connect(nGain);
        ...
        for(let i=0; i<=60; i++) {
            ...
            f1.frequency.linearRampToValueAtTime(Math.max(50, 200 + (1-y)*600 - th*50), time);
            f2.frequency.linearRampToValueAtTime(800 + x*1400, time);
            f3.frequency.linearRampToValueAtTime(2000 + l*1500, time);
            nasF.frequency.linearRampToValueAtTime(10000 - n*9000, time);
        }

        sNode.connect(f1); nGain.connect(f1);
        f1.connect(f2); f2.connect(f3); f3.connect(nasF); nasF.connect(offline.destination);
    ...
    const handlePlayPauseSim = async () => {
        ...
        const res = await renderAdvancedAudio();
        if (!res) return;
        ...
        const startOffset = pauseOffsetRef.current % res.duration;
    ...
    const handleCanvasMouseDown = (e) => {
        ...
        if (my < RULER_HEIGHT) { 
            setPlayHeadPos(t); 
            pauseOffsetRef.current = t * advDuration; 
        ...
        const hitIndex = track.points.findIndex(p => {
            const px = p.t * rect.width;
            const py = RULER_HEIGHT + (1 - (p.v - track.min) / (track.max - track.min)) * graphH;
        ...
        if (hitIndex !== -1) {
            setDraggingKeyframe({ index: hitIndex, trackId: selectedTrackId });
        } else {
    ...
    const handleCanvasContextMenu = (e) => {
        ...
        if (hitIndex !== -1) {
            ...
            setAdvTracks(prev => prev.map(tr => {
                ...
                const newPoints = tr.points.filter((_, i) => i !== hitIndex);
                return { ...tr, points: newPoints };
            }));
        }
    };
    ...
    useEffect(() => {
        const move = (e) => {
            ...
            const t = Math.max(0, Math.min(1, mx / rect.width));
            const rawV = track.min + (1 - (my - RULER_HEIGHT) / graphH) * (track.max - track.min);
            const v = Math.max(track.min, Math.min(track.max, rawV));

            setAdvTracks(prev => prev.map(tr => {
                if (tr.id !== draggingKeyframe.trackId) return tr;
                const newPoints = [...tr.points];
                newPoints[draggingKeyframe.index] = { t, v };
                return { ...tr, points: newPoints.sort((a,b) => a.t - b.t) };
            }));
        };
    ...
    useEffect(() => {
        ...
        for(let i=0; i<=10; i++) { const x = i*(w/10); ctx.moveTo(x, RULER_HEIGHT); ctx.lineTo(x, h); }
        ctx.stroke();
        ...
        const phX = playHeadPos * w; ctx.strokeStyle = '#ef4444'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(phX,0); ctx.lineTo(phX,h); ctx.stroke();
    }, [selectedTrackId, advTracks, playHeadPos]);

    const updateTrackAtPlayhead = (trackId, val) => {
        setAdvTracks(prev => prev.map(tr => {
            if (tr.id !== trackId) return tr;
            const threshold = 0.05; 
            const existingIdx = tr.points.findIndex(p => Math.abs(p.t - playHeadPos) < threshold);
            let newPoints = [...tr.points];
            if (existingIdx !== -1) { newPoints[existingIdx] = { ...newPoints[existingIdx], v: val }; } 
    ...
    return (
        <div className="flex-1 flex flex-col p-4 gap-4 animate-in fade-in overflow-hidden font-sans" onMouseUp={() => { if(animRef.current) cancelAnimationFrame(animRef.current); setDragPart(null); }}>
            <div className="flex-[3] flex gap-4 min-h-0 overflow-hidden">
                <div className="flex-1 bg-white/60 rounded-2xl border border-slate-300 relative overflow-hidden shadow-sm flex flex-col">
                    <div className="flex-1 relative flex items-center justify-center p-4 bg-slate-100/50">
                        {/* Adjusted ViewBox for better ratio & larger size (max-w-[230px]) */}
                        <svg viewBox="0 0 400 400" className="w-full h-full max-w-[230px] max-h-[230px] drop-shadow-2xl">
                            {/* Head/Nose/Throat Context Outline */}
                            <path d="M 50 250 Q 50 100 200 100 Q 350 100 350 250 L 350 400 L 50 400 Z" fill="none" stroke="#e2e8f0" strokeWidth="4" />
                            <path d="M 350 220 Q 380 220 390 240" fill="none" stroke="#cbd5e1" strokeWidth="3" /> {/* Nose hint */}
                            
                            {/* Throat Pipe (Separate from Tongue) */}
                            {/* Visualizing Throat Constriction separately at bottom */}
                            <path d="M 120 400 L 120 600" stroke="#94a3b8" strokeWidth={Math.max(2, 40 - liveTract.throat * 30)} strokeLinecap="round" opacity="0.5" />

                            {/* Vocal Tract / Tongue (Stroke width constant) */}
                            ...
                            {/* Draggable Lip Ellipse */}
                            <ellipse 
                                cx={350 + liveTract.lipLen * 20} 
                                cy="225" 
                                rx={6 + liveTract.lipLen * 30} 
                                ry={3 + liveTract.lips * 40} 
                                fill="#db2777" 
                                opacity="0.85" 
                        ...
                        <div className="absolute inset-0" 
                            onMouseMove={(e) => {
                                ...
                                if (dragPart === 'lips') {
                                    setLiveTract(p => ({...p, lipLen: x, lips: y}));
                                } else if (dragPart === 'tongue') {
                                    setLiveTract(p => ({...p, x: x, y: y}));
                                }
                            }} 
                            onMouseDown={(e) => {
                                if (dragPart) return; 
                                setManualPose(true); // Enable manual pose mode
                                const rect = e.currentTarget.getBoundingClientRect();
                                const normX = (e.clientX - rect.left) / rect.width;
                                const normY = (e.clientY - rect.top) / rect.height;
                                // Improved Hit detection for Lips and Tongue
                                // Lips: approx 0.88, 0.56 in normalized coords
                                if (Math.hypot(normX - 0.88, normY - 0.56) < 0.3) {
                                    setDragPart('lips');
                                } else if (normX > 0.35 && normX < 0.75 && normY > 0.5 && normY < 0.95) {
                                    // Bounding box for Tongue
                                    setDragPart('tongue');
                                    setLiveTract(p => ({...p, x: normX, y: 1-normY}));
                                }
                            }} 
                            onMouseUp={() => setDragPart(null)} 
                            onMouseLeave={() => setDragPart(null)}
                        />
                    </div>
                    <div className="p-4 bg-slate-50/80 border-t border-slate-200 flex justify-between items-center backdrop-blur-md">
                        <div className="flex gap-2">
                            <button onClick={handleSimUndo} disabled={simUndoStack.length === 0} title="실행 취소" className="p-2 bg-white rounded-xl border border-slate-300 disabled:opacity-30"><Undo2 size={18}/></button>
                            <button onClick={() => { pushSimUndo(); setAdvTracks(prev => prev.map(t => ({...t, points: [{t:0,v:t.id==='pitch'?220:0.5},{t:1,v:t.id==='pitch'?220:0.5}]}))); setManualPose(false); }} title="초기화" className="p-2 bg-white rounded-xl border border-slate-300 text-red-500"><RotateCcw size={18}/></button>
                        </div>
                        <div className="flex gap-2">
                             <button onClick={registerKeyframe} className="bg-[#209ad6] hover:bg-[#1a85b9] text-white px-4 py-2 rounded-xl font-bold text-xs shadow-lg active:scale-95 transition-all flex items-center gap-2">
                                <CircleDot size={14}/> 키프레임 등록
                             </button>
                             <button onClick={handlePlayPauseSim} className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs shadow-sm active:scale-95 transition-all flex items-center gap-2">
                                {isAdvPlaying ? <Pause size={14}/> : <Play size={14}/>} {isAdvPlaying ? '일시정지' : '재생'}
                             </button>
                             <button onClick={async()=>{ const res = await renderAdvancedAudio(); if(res) onAddToRack(res, "시뮬레이션_결과"); }} className="bg-[#a3cef0] hover:bg-[#209ad6] hover:text-white text-[#1f1e1d] px-4 py-2 rounded-xl font-bold text-xs shadow-lg active:scale-95 transition-all flex items-center gap-1"><LogIn size={14}/> 보관함에 저장</button>
                        </div>
                    </div>
                </div>
                <div className="w-72 bg-white/40 rounded-2xl border border-slate-300 p-3 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
                    <h3 className="font-black text-slate-600 uppercase tracking-widest flex items-center gap-2 text-[10px]"><Sliders size={18} className="text-[#209ad6]"/> 파라미터</h3>
                    <div className="space-y-3">
                        ...
                        <div className="flex gap-2 mb-4">
                             <button onClick={()=>{setAdvTracks(prev=>prev.map(t=>t.id==='pitch'?{...t,points:[{t:0,v:110},{t:1,v:110}]}:t))}} className="flex-1 py-1.5 bg-white border border-slate-300 rounded-lg text-blue-500 text-xs font-bold hover:bg-blue-50">Male</button>
                             <button onClick={()=>{setAdvTracks(prev=>prev.map(t=>t.id==='pitch'?{...t,points:[{t:0,v:330},{t:1,v:330}]}:t))}} className="flex-1 py-1.5 bg-white border border-slate-300 rounded-lg text-pink-500 text-xs font-bold hover:bg-pink-50">Female</button>
                        </div>
                        {[{id:'lips', label:'입술 열기'}, {id:'lipLen', label:'입술 길이'}, {id:'throat', label:'목 조임'}, {id:'nasal', label:'비성'}].map(p => (
                            <div key={p.id} className="space-y-1">
                                <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase"><span>{p.label}</span><span>{Math.round(liveTract[p.id]*100)}%</span></div>
                                <input type="range" min="0" max="1" step="0.01" value={liveTract[p.id]} 
                                    ...
                                    className="w-full h-1 bg-slate-300 appearance-none rounded-full accent-[#209ad6]"/>
                            </div>
                        ))}
                        <div className="pt-2 border-t border-slate-200">
                             <div className="flex justify-between text-[10px] font-bold text-emerald-600 uppercase"><span>시뮬레이션 강도 (과장)</span><span>{Math.round(intensity*100)}%</span></div>
                             <input type="range" min="0" max="3" step="0.1" value={intensity} onChange={e=>setIntensity(Number(e.target.value))} className="w-full h-1 bg-slate-300 appearance-none rounded-full accent-emerald-500"/>
                        </div>
                        <div className="space-y-1 mt-2">
                             <span className="text-[9px] font-bold text-slate-400 uppercase">입력 소스 (Base)</span>
                             <select value={tractSourceFileId} onChange={e=>setTractSourceFileId(e.target.value)} className="w-full text-[10px] p-1.5 rounded border border-slate-200">
                                 <option value="">기본 신디사이저</option>
                                 {files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
                             </select>
                        </div>
                        <div className="space-y-1">
                             <span className="text-[9px] font-bold text-slate-400 uppercase">노이즈 소스 (Noise)</span>
                             <select value={noiseSourceFileId} onChange={e=>setNoiseSourceFileId(e.target.value)} className="w-full text-[10px] p-1.5 rounded border border-slate-200">
                                 <option value="">기본 화이트 노이즈</option>
                                 {files.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
                             </select>
                        </div>
...
const App = () => {
    ...
    const handleFileUpload = async (e) => {
        ...
        for(const file of selFiles) {
            ...
            setFiles(prev => [...prev, { id: Math.random().toString(36).substr(2, 9), name: file.name, buffer }]);
        }
    };

    // Helper functions for file management (Fixed: Added to App scope)
    ...
    const exportProject = async () => {
        const data = {
            files: await Promise.all(files.map(async f => ({ id: f.id, name: f.name, data: AudioUtils.serializeBuffer(f.buffer) }))),
            exportDate: new Date().toISOString()
        };
        const blob = new Blob([JSON.stringify(data)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url; a.download = `otonashi_project_${new Date().getTime()}.json`; a.click();
    };
        ...
        <div className="h-screen w-full bg-[#f8f8f6] text-[#1f1e1d] flex flex-col font-sans overflow-hidden">
            ...
            <header className="h-14 border-b border-slate-300 bg-white flex items-center justify-between px-6 shrink-0 z-10 shadow-sm font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                <div className="flex items-center gap-3 font-sans">
                    <div className="bg-[#209ad6] p-1.5 rounded-lg text-white shadow-lg shadow-blue-200 font-sans font-sans font-sans"><Activity size={20}/></div>
                    <div className="flex flex-col font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                        <h1 className="font-black text-xl tracking-tighter leading-none bg-clip-text text-transparent bg-gradient-to-r from-[#b2d4ed] via-[#3c78e8] to-[#e3daf5] font-sans font-sans font-sans font-sans font-sans font-sans">OTONASHI</h1>
                ...
                <nav className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                    <button onClick={()=>setActiveTab('editor')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab==='editor'?'bg-white text-[#209ad6] shadow-sm border border-slate-200 font-sans font-sans font-sans font-sans font-sans font-sans font-sans':'text-slate-500 hover:text-slate-800 font-sans font-sans font-sans font-sans font-sans font-sans font-sans'}`}>스튜디오</button>
                    <button onClick={()=>setActiveTab('consonant')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab==='consonant'?'bg-white text-[#209ad6] shadow-sm border border-slate-200 font-sans font-sans font-sans font-sans font-sans font-sans font-sans':'text-slate-500 hover:text-slate-800 font-sans font-sans font-sans font-sans font-sans font-sans font-sans'}`}>자음 합성</button>
                    <button onClick={()=>setActiveTab('sim')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab==='sim'?'bg-white text-[#209ad6] shadow-sm border border-slate-200 font-sans font-sans font-sans font-sans font-sans font-sans font-sans':'text-slate-500 hover:text-slate-800 font-sans font-sans font-sans font-sans font-sans font-sans font-sans'}`}>성도 시뮬레이터</button>
                </nav>
                <div className="flex items-center gap-3 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><button className="text-slate-400 hover:text-slate-600 transition-colors font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Settings size={20}/></button><div className="w-8 h-8 rounded-full bg-slate-200 border border-slate-300 overflow-hidden flex items-center justify-center shadow-inner font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><User size={20} className="text-slate-400 font-sans font-sans font-sans font-sans font-sans font-sans"/></div></div>
            </header>
            <main className="flex-1 flex overflow-hidden font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-hidden font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
