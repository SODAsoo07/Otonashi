...
import { CircleDot, Download, Pause, Play, Redo2, Save, Undo2 } from 'lucide-react';
...
const TractVisualizer: React.FC<TractVisualizerProps> = ({
    ...
    const text = TRACT_TEXT[language];
        ...
        <div className="w-full h-full bg-white/60 dynamic-radius border border-slate-300 flex flex-col relative overflow-hidden shadow-sm min-h-0">
            <div className="flex-1 relative flex items-center justify-center overflow-hidden py-[3px]">
                <div
                    className={
                        size === 'compact'
                            ? 'h-[70%] w-auto max-w-[340px] max-h-[420px] min-w-[160px] min-h-[200px] flex items-center justify-center aspect-[14/17]'
                            : 'h-[80%] w-auto max-w-[420px] max-h-[520px] min-w-[200px] min-h-[260px] flex items-center justify-center aspect-[14/17]'
                    }
                >
            ...
            <div className="mt-auto p-2 px-4 bg-white/80 border-t flex justify-between items-center shrink-0 shadow-inner">
                <div className="flex gap-2">
                    ...
                    <button onClick={onRedo} disabled={redoStackLength === 0} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 disabled:opacity-20 transition-all shadow-sm">
                        <Redo2 size={16} />
                    </button>
                </div>
                <div className="flex gap-1.5 font-bold text-xs items-center">
                    <button onClick={onRecordSnapshot} className="dynamic-primary text-slate-900 px-3 py-1.5 rounded-full text-xs font-black flex items-center gap-1.5 shadow-md active:scale-95 transition-all">
                        <CircleDot size={14} /> {text.record}
                    </button>
                    <div className="w-px h-4 bg-slate-200 mx-1" />
                    <button onClick={onPlayToggle} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-md active:scale-95 transition-all font-black">
                        {isAdvPlaying ? <Pause size={14} /> : <Play size={14} />} {isAdvPlaying ? text.stop : text.play}
                    </button>
                    <button onClick={onDownload} className="bg-green-50 text-green-700 border border-green-200 px-3 py-1.5 rounded-lg hover:bg-green-100 shadow-sm transition-all flex items-center gap-1.5 font-black">
                        <Download size={14} /> WAV
                    </button>
                    <button onClick={onSaveToRack} className="bg-white border border-slate-300 px-3 py-1.5 rounded-lg hover:bg-slate-50 shadow-sm active:scale-95 transition-all font-black flex items-center gap-1.5">
                        <Save size={14} /> {text.saveToRack}
                    </button>
                </div>
            </div>
        </div>
