                    ...
                    <div className="flex-1 relative flex items-center justify-center p-4 min-h-0 font-sans">
                        <div className="relative w-full max-w-[450px] aspect-square">
                            ...
                            <div className="absolute inset-0 z-20">
                                ...
                                >
                                    <span className="text-xs text-rose-400 font-bold opacity-0 group-hover:opacity-100 transition-opacity select-none">혀 (Tongue)</span>
                                </div>
                                {/* Lip Area */}
                                <div 
                                ...
                                >
                                    <MoveHorizontal className="w-6 h-6 text-emerald-300 mb-1"/>
                                    <span className="text-[10px] text-emerald-400 font-black uppercase tracking-widest text-center px-1 select-none">입술</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="p-2 px-4 bg-white/80 border-t flex justify-between items-center shrink-0 font-sans">
                        <button onClick={()=>{const t=playHeadPos; const track = advTracks.find(t=>t.id===selectedTrackId); if(!track) return; setAdvTracks(prev=>prev.map(tr=>{if(tr.group!=='adj')return tr;let val=null;if(track.id==='tongueX')val=liveTract.x;else if(track.id==='tongueY')val=liveTract.y;else if(track.id==='lips')val=liveTract.lips;else if(track.id==='lipLen')val=liveTract.lipLen;else if(track.id==='throat')val=liveTract.throat;else if(track.id==='nasal')val=liveTract.nasal;if(val===null)return tr;return{...tr,points:[...tr.points.filter(p=>Math.abs(p.t-t)>0.005),{t,v:val}].sort((a,b)=>a.t-b.t)};}));}} className="bg-[#209ad6] hover:bg-[#1a85b9] text-white px-4 py-1.5 rounded-full text-[11px] font-bold flex items-center gap-1.5 active:scale-95 transition-all font-sans font-bold"><CircleDot className="w-3.5 h-3.5"/> 기록</button>
                        <div className="flex gap-1.5 font-black uppercase text-[11px] font-bold"><button onClick={handleSimulationPlay} className="bg-slate-800 text-white px-4 py-1.5 rounded-lg flex items-center gap-2 font-bold">{isAdvPlaying ? <Pause size={14} fill="currentColor"/> : <Play size={14} fill="currentColor"/>} {isAdvPlaying ? '일시정지' : '재생'}</button><button onClick={async()=>{ const res = await renderAdvancedAudio(); if(res) onAddToRack(res, "시뮬레이션_" + simIndex); setSimIndex(si => si + 1); }} className="bg-white border border-slate-200 text-slate-700 px-3 py-1.5 rounded-lg transition-colors font-bold font-sans">보관함에 저장</button></div>
                    </div>
                </div>
                <div className="w-80 bg-white/40 rounded-2xl border border-slate-300 p-4 flex flex-col gap-4 overflow-y-auto shrink-0 custom-scrollbar font-sans font-bold">
                    <h3 className="font-black text-slate-600 uppercase tracking-widest flex items-center gap-2 text-[11px] font-bold font-sans font-black font-sans"><Sliders size={20} className="text-[#209ad6]"/> 설정</h3>
                    <div className="space-y-4 font-bold text-slate-500 uppercase text-[10px] font-sans font-bold">
                        {[ {id:'lips', label:'입술 열기', color:'accent-pink-400'}, {id:'lipLen', label:'입술 길이', color:'accent-pink-600'}, {id:'throat', label:'목 조임', color:'accent-purple-400'}, {id:'nasal', label:'비성 (콧소리)', color:'accent-orange-400'} ].map(p => (
                            <div key={p.id} className="space-y-1.5 font-sans font-bold">
                                ...
                                {/* @ts-ignore dynamic index */}
                                <input type="range" min="0" max="1" step="0.01" value={liveTract[p.id]} onChange={e=>{ const v=Number(e.target.value); const n = {...liveTract, [p.id]:v}; setLiveTract(n); updateLiveAudio(n.x, n.y, n.lips, n.throat, n.lipLen, n.nasal); }} className={`w-full h-1.5 bg-slate-300 appearance-none rounded-full font-bold ${p.color}`}/>
                            </div>
                        ))}
                        <div className="space-y-1.5 font-sans font-bold"><div className="flex justify-between font-bold font-sans"><span>숨소리 (Breath)</span><span className="text-xs">{Math.round(larynxParams.breathGain*100)}%</span></div><input type="range" min="0" max="1" step="0.01" value={larynxParams.breathGain} onChange={e=>setLarynxParams(p=>({...p, breathGain: Number(e.target.value)}))} className="w-full h-1.5 bg-slate-300 appearance-none rounded-full accent-cyan-400 font-bold"/></div>
                        <div className="h-px bg-slate-200 my-2 font-sans"></div>
                        
                        <div className="space-y-1.5 font-sans font-bold font-black uppercase font-sans"><span className="text-[10px] font-bold font-sans">음원 소스 (Base)</span><select value={tractSourceType} onChange={e=>setTractSourceType(e.target.value)} className="w-full bg-white border border-slate-200 rounded p-1.5 outline-none font-bold text-[10px] font-sans font-black font-sans"><option value="synth">기본 신디사이저</option><option value="file">보관함 파일</option></select></div>
                        {tractSourceType==='synth' && (
                          <div className="space-y-2 font-sans font-bold font-sans font-bold">
                            <div className="grid grid-cols-2 gap-1.5 font-sans font-bold">
                              {['sawtooth', 'sine', 'square', 'complex', 'noise'].map(t=>(<button key={t} onClick={()=>setSynthWaveform(t)} className={`py-1.5 rounded border text-[9px] font-black font-sans ${synthWaveform===t?'bg-indigo-500 text-white border-indigo-500 font-black':'bg-white text-slate-500'}`}>{t.toUpperCase()}</button>))}
                            </div>
                            <div className="space-y-1.5 font-sans font-bold">
                              ...
                              <input type="range" min="0.05" max="0.95" step="0.01" value={pulseWidth} onChange={e=>setPulseWidth(Number(e.target.value))} className="w-full h-1.5 bg-slate-300 appearance-none rounded-full accent-indigo-500 font-sans"/>
                            </div>
                          </div>
                        )}
                        {tractSourceType==='file' && <select value={tractSourceFileId} onChange={e=>setTractSourceFileId(e.target.value)} className="w-full bg-white border border-slate-200 rounded p-1.5 text-[10px] font-sans font-bold font-sans font-bold">{files.map(f=><option key={f.id} value={f.id} className="font-sans font-bold font-sans font-bold">{f.name}</option>)}</select>}
                        
                        <div className="space-y-1.5 font-sans font-bold font-black uppercase font-sans font-bold"><span className="text-[10px] text-slate-500 font-bold font-sans">노이즈 소스 (Noise)</span><select value={larynxParams.noiseSourceType} onChange={e=>setLarynxParams({...larynxParams, noiseSourceType:e.target.value})} className="w-full bg-white border border-slate-200 rounded p-1.5 outline-none font-bold text-[10px] font-sans font-bold font-sans font-bold font-black"><option value="generated">기본 화이트 노이즈</option><option value="file">보관함 파일</option></select></div>
                        {larynxParams.noiseSourceType==='file' && <select value={larynxParams.noiseSourceFileId} onChange={e=>setLarynxParams({...larynxParams, noiseSourceFileId:e.target.value})} className="w-full bg-white border border-slate-200 rounded p-1.5 text-[10px] mt-1 font-sans font-bold font-sans font-bold">{files.map(f=><option key={f.id} value={f.id} className="font-sans font-bold">{f.name}</option>)}</select>}
                    </div>
                </div>
            </div>
