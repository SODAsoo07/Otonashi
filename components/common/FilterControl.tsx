    ...
    {state.on && (
      <div className="space-y-1">
        <input 
          ...
          value={state.freq} 
          onChange={e=>onChange({...state, freq: Number(e.target.value)})} 
          className="w-full h-1.5 bg-slate-200 rounded-full appearance-none accent-indigo-500 cursor-pointer"
        />
        <div className="flex items-center gap-2">
          ...
          <input 
            ...
            className="flex-1 h-1.5 bg-slate-200 rounded-full appearance-none accent-slate-400 cursor-pointer"
          />
          <input 
            type="number" 
            step="0.1"
            value={state.q} 
            onChange={e => onChange({...state, q: Number(e.target.value)})} 
            className="w-10 text-[9px] text-slate-500 font-mono bg-transparent text-right outline-none font-bold"
          />
        </div>
      </div>
    )}
