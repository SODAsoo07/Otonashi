...
const ANALYZER_UI_TEXT = {
    ko: {
        title: 'AI 발음 분석',
        calibration: '사용자 맞춤 보정',
        calibrationPlaceholder: '보정용 파일 (아/이/우 포함) 선택...',
        calibrationRun: '보정 실행',
        calibrationRunning: '보정 중...',
        calibrationAnalyzing: '캘리브레이션 분석 중...',
        calibrationDone: (hz: number) => `보정 완료 (A: ${hz}Hz)`,
        calibrationError: '오류 발생',
        analyzeFile: '분석할 파일',
        filePlaceholder: '파일 선택...',
        analysisLanguage: '발음 언어',
        langJP: '일본어 (JP)',
        langKR: '한국어 (KR)',
        langEN: '영어 (EN)',
        analyze: '분석',
        idleHint: '오디오 파일을 선택하고 분석을 시작하세요',
        generationParams: '생성 파라미터',
        motionSmoothing: '모션 스무딩',
        analysisSensitivity: '분석 민감도',
        consonantDetection: '자음 탐지 강화 (M/P, S/Sh)',
        consonantHint: '자음 탐지를 켜면 에너지 급감 구간에서 입술 닫기(M/P)가, 고주파 잡음 구간에서 치조음(S) 모션이 자동으로 추가됩니다.',
        overwriteWarning: '주의: 기존 트랙의 키프레임이 덮어씌워집니다.',
        cancel: '취소',
        apply: '확인 및 적용',
        createTracksApply: '트랙 생성 및 적용',
        durationError: '분석 부하를 줄이기 위해 7초 이하의 파일만 지원합니다.',
    },
    en: {
        title: 'AI Pronunciation Analyzer',
        calibration: 'Calibration',
        calibrationPlaceholder: 'Select a calibration file (A/I/U)...',
        calibrationRun: 'Run Calibration',
        calibrationRunning: 'Calibrating...',
        calibrationAnalyzing: 'Analyzing calibration sample...',
        calibrationDone: (hz: number) => `Calibration complete (A: ${hz}Hz)`,
        calibrationError: 'An error occurred',
        analyzeFile: 'File to analyze',
        filePlaceholder: 'Select a file...',
        analysisLanguage: 'Pronunciation language',
        langJP: 'Japanese (JP)',
        langKR: 'Korean (KR)',
        langEN: 'English (EN)',
        analyze: 'Analyze',
        idleHint: 'Select an audio file and start the analysis',
        generationParams: 'Generation Parameters',
        motionSmoothing: 'Motion Smoothing',
        analysisSensitivity: 'Analysis Sensitivity',
        consonantDetection: 'Enhanced consonant detection (M/P, S/Sh)',
        consonantHint: 'When enabled, the analyzer adds lip closure for M/P-like valleys and sibilant motion for high-frequency noise regions.',
        overwriteWarning: 'Warning: existing keyframes in these tracks will be overwritten.',
        cancel: 'Cancel',
        apply: 'Confirm & Apply',
        createTracksApply: 'Create Tracks & Apply',
        durationError: 'Only files up to 7 seconds are supported to keep analysis responsive.',
    },
    ja: {
        title: 'AI 発音アナライザー',
        calibration: 'ユーザー補正',
        calibrationPlaceholder: '補正用ファイル（あ/い/うを含む）を選択...',
        calibrationRun: '補正を実行',
        calibrationRunning: '補正中...',
        calibrationAnalyzing: '補正サンプルを解析中...',
        calibrationDone: (hz: number) => `補正完了 (A: ${hz}Hz)`,
        calibrationError: 'エラーが発生しました',
        analyzeFile: '解析するファイル',
        filePlaceholder: 'ファイルを選択...',
        analysisLanguage: '発音言語',
        langJP: '日本語 (JP)',
        langKR: '韓国語 (KR)',
        langEN: '英語 (EN)',
        analyze: '解析',
        idleHint: '音声ファイルを選択して解析を開始してください',
        generationParams: '生成パラメータ',
        motionSmoothing: 'モーションの平滑化',
        analysisSensitivity: '解析感度',
        consonantDetection: '子音検出を強化 (M/P, S/Sh)',
        consonantHint: '有効にすると、エネルギーが落ちる区間で唇閉鎖 (M/P) を、高周波ノイズ区間で歯擦音 (S) モーションを自動で追加します。',
        overwriteWarning: '注意: 既存トラックのキーフレームは上書きされます。',
        cancel: 'キャンセル',
        apply: '確認して適用',
        createTracksApply: 'トラックを作成して適用',
        durationError: '解析を快適に保つため、7秒以下のファイルのみ対応しています。',
    },
} as const;

const FormantAnalyzer: React.FC<FormantAnalyzerProps> = ({ files, audioContext, onClose, onApply }) => {
    const { language: uiLanguage } = useLanguage();
    const uiText = ANALYZER_UI_TEXT[uiLanguage];
    ...
    // Params
    const [language, setLanguage] = useState<LanguageMode>(() => uiLanguage === 'ko' ? 'KR' : uiLanguage === 'ja' ? 'JP' : 'EN');
    ...
    useEffect(() => {
        ...
        if (selectedId) {
            ...
            if (f && f.buffer.duration > 7.0) setErrorMsg(uiText.durationError);
            if (f && useFilenameHint) setDetectedHints(extractVowelsFromName(f.name));
        } else {
            setDetectedHints([]);
        }
    }, [selectedId, useFilenameHint, files, uiText]);
    ...
    const runCalibration = async () => {
        ...
        setCalibMsg(uiText.calibrationAnalyzing);
        ...
        setTimeout(() => {
            try {
                ...
                setCalibMsg(uiText.calibrationDone(Math.round(avgA.f1)));
                setStatus('idle');
            } catch (e) {
                setCalibMsg(uiText.calibrationError);
                setStatus('idle');
            }
        }, 100);
    };
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-[700px] flex flex-col overflow-hidden font-sans border border-slate-200 max-h-[90vh]">
                <div className="p-4 bg-slate-50 border-b flex justify-between items-center shrink-0">
                    <h3 className="font-black text-slate-700 flex items-center gap-2"><Wand2 size={18} className="text-purple-500"/> {uiText.title}</h3>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
                    ...
                    <div className={`p-4 rounded-xl border transition-all ${useCalibration ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'}`}>
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <button onClick={()=>setUseCalibration(!useCalibration)} disabled={!hasCalibration} className={`transition-colors ${useCalibration ? 'text-green-600' : 'text-slate-300'} disabled:opacity-50`}>
                        ...
                        <div className="flex gap-2 items-end">
                            <select value={calibFileId} onChange={e=>setCalibFileId(e.target.value)} className="flex-1 p-2 bg-white border border-slate-300 rounded-lg text-xs font-bold outline-none">
                                <option value="">{uiText.calibrationPlaceholder}</option>
                            ...
                            <button onClick={runCalibration} disabled={!calibFileId || status === 'calibrating'} className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg text-xs font-black transition-colors disabled:opacity-50">
                                {status === 'calibrating' ? <Activity className="animate-spin" size={14}/> : uiText.calibrationRun}
                            </button>
                        </div>
                    ...
                    <div className="flex gap-2 items-end">
                        <div className="flex-1 space-y-1">
                            <label className="text-xs font-black text-slate-500 uppercase tracking-widest">{uiText.analyzeFile}</label>
                            <select value={selectedId} onChange={e=>{setSelectedId(e.target.value); setIsConfirming(false);}} className="w-full p-2 bg-slate-100 border border-slate-200 rounded-lg text-sm font-bold outline-none focus:ring-2 ring-purple-200">
                                <option value="">{uiText.filePlaceholder}</option>
                                {files.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
                            </select>
                        ...
                        <div className="space-y-1 w-28">
                             <label className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-1"><Languages size={12}/> {uiText.analysisLanguage}</label>
                             <select value={language} onChange={e=>setLanguage(e.target.value as LanguageMode)} className="w-full p-2 bg-slate-100 border border-slate-200 rounded-lg text-sm font-black outline-none">
                                <option value="JP">{uiText.langJP}</option>
                                <option value="KR">{uiText.langKR}</option>
                                <option value="EN">{uiText.langEN}</option>
                             </select>
                        </div>
                        <button onClick={analyze} disabled={!selectedId || status === 'analyzing' || !!errorMsg} className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-black text-sm flex items-center gap-2 transition-all disabled:opacity-50 h-10 shadow-md">
                            {status === 'analyzing' ? <Activity className="animate-spin" size={16}/> : <Play size={16}/>} {uiText.analyze}
                        </button>
                    </div>
                    ...
                    <div className="bg-slate-900 rounded-xl overflow-hidden relative h-[220px] shadow-inner border border-slate-700 group shrink-0">
                        {status === 'idle' && <div className="absolute inset-0 flex items-center justify-center text-slate-500 text-xs font-bold">{uiText.idleHint}</div>}
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <h4 className="text-xs font-black text-slate-600 uppercase tracking-wider">{uiText.generationParams}</h4>
                        </div>
                        ...
                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                            <div className="flex gap-4">
                                <div className="flex-1 space-y-1">
                                    <div className="flex justify-between text-xs font-bold text-slate-600">
                                        <span>{uiText.motionSmoothing}</span>
                                ...
                                <div className="flex-1 space-y-1">
                                    <div className="flex justify-between text-xs font-bold text-slate-600">
                                        <span>{uiText.analysisSensitivity}</span>
                            ...
                            <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-200">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-black text-slate-700">{uiText.consonantDetection}</span>
                                </div>
                    ...
                    {isConfirming ? (
                         <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2 w-full justify-between bg-red-50 px-3 py-2 rounded-lg border border-red-100">
                            <div className="flex items-center gap-2 text-xs font-black text-red-600">
                                <AlertTriangle size={16}/> {uiText.overwriteWarning}
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => setIsConfirming(false)} className="px-3 py-1.5 bg-white text-slate-500 hover:bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold shadow-sm transition-colors">{uiText.cancel}</button>
                                <button onClick={confirmApply} className="px-4 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs font-black shadow-md transition-all active:scale-95">{uiText.apply}</button>
                            </div>
                        </div>
                    ) : (
                        <>
                            <button onClick={onClose} className="px-4 py-2 text-slate-500 hover:bg-slate-200 rounded-lg text-sm font-bold transition-colors">{uiText.cancel}</button>
                            <button onClick={handleApplyRequest} disabled={status !== 'done'} className="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-black flex items-center gap-2 transition-all disabled:opacity-50 shadow-lg active:scale-95">
                                <Layers size={16}/> {uiText.createTracksApply}
                            </button>
                        </>
                    )}
                </div>
