...
const MiscTab: React.FC<MiscTabProps> = ({ audioContext, files, onAddToRack, isActive, monitorGainValue = 1.0 }) => {
    ...
    useEffect(() => {
        if (monitorNodeRef.current && audioContext) {
            monitorNodeRef.current.gain.setTargetAtTime(monitorGainValue, audioContext.currentTime, 0.05);
        }
    }, [monitorGainValue, audioContext]);
    ...
    const playAudio = async () => {
        ...
        masterGain.connect(monitorNode);
