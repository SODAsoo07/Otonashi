...
import { Wand2, Play, Save, Sliders, Activity, Volume2, Waves, RotateCcw } from 'lucide-react';
...
const ConsonantGeneratorTab: React.FC<ConsonantGeneratorTabProps> = ({ audioContext, onAddToRack }) => {
    ...
    const loadPreset = (type: 's' | 'sh' | 't' | 'k' | 'h') => {
        if(type === 's') {
            setFilterType('highpass'); setFrequency(4500); setQ(2); setDuration(250); setAttack(20); setDecay(50); setSustain(0.8); setRelease(100); setNoiseType('white');
        } else if (type === 'sh') {
            setFilterType('bandpass'); setFrequency(2500); setQ(1.5); setDuration(250); setAttack(30); setDecay(50); setSustain(0.8); setRelease(100); setNoiseType('white');
        } else if (type === 't') {
            setFilterType('highpass'); setFrequency(3000); setQ(1); setDuration(80); setAttack(2); setDecay(15); setSustain(0); setRelease(20); setNoiseType('white');
        } else if (type === 'k') {
            setFilterType('lowpass'); setFrequency(1500); setQ(1); setDuration(100); setAttack(5); setDecay(30); setSustain(0); setRelease(30); setNoiseType('pink');
        } else if (type === 'h') {
            setFilterType('bandpass'); setFrequency(1000); setQ(0.5); setDuration(200); setAttack(40); setDecay(50); setSustain(0.6); setRelease(80); setNoiseType('pink');
        }
    };
    ...
    const generateAudio = async () => {
        ...
        const tSus = Math.max(0, totalDurationSec - tAtt - tDec - (release / 1000));
        const tRel = release / 1000;

        amp.gain.setValueAtTime(0, t0);
        amp.gain.linearRampToValueAtTime(gain, t0 + tAtt);
        amp.gain.linearRampToValueAtTime(gain * sustain, t0 + tAtt + tDec);
        // Sustain holds...
        amp.gain.setValueAtTime(gain * sustain, Math.max(t0 + tAtt + tDec, totalDurationSec - tRel));
    ...
    useEffect(() => {
        const draw = async () => {
            // Generate a preview buffer for visualization if not generated yet
            const buf = generatedBuffer || await generateAudio();
            ...
            // Draw Envelope Overlay (Approximate)
            ctx.beginPath();
            ...
            ctx.moveTo(0, h);
            ctx.lineTo(pxAtt, h - (h * gain * 0.9)); // Attack Peak
            ctx.lineTo(pxAtt + pxDec, h - (h * gain * sustain * 0.9)); // Decay to Sustain
            if (pxSus > 0) ctx.lineTo(pxAtt + pxDec + pxSus, h - (h * gain * sustain * 0.9)); // Sustain
            ctx.lineTo(w, h); // Release to 0
            ctx.stroke();
        };
        draw();
    }, [duration, attack, decay, sustain, release, filterType, frequency, Q, gain, noiseType, generatedBuffer]);

    // Invalidate buffer when params change
    useEffect(() => {
        setGeneratedBuffer(null);
    }, [duration, attack, decay, sustain, release, filterType, frequency, Q, gain, noiseType]);
            ...
            <div className="bg-white/60 rounded-3xl border border-slate-300 p-8 flex flex-col gap-6 shadow-sm h-full">
                 <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                    ...
                    <div className="flex gap-2">
                        <button onClick={()=>loadPreset('t')} className="px-3 py-1 bg-white border rounded-lg text-xs hover:bg-slate-50 font-bold">T (Plosive)</button>
                        <button onClick={()=>loadPreset('k')} className="px-3 py-1 bg-white border rounded-lg text-xs hover:bg-slate-50 font-bold">K (Plosive)</button>
                        <button onClick={()=>loadPreset('s')} className="px-3 py-1 bg-white border rounded-lg text-xs hover:bg-slate-50 font-bold">S (Sibilant)</button>
                        <button onClick={()=>loadPreset('sh')} className="px-3 py-1 bg-white border rounded-lg text-xs hover:bg-slate-50 font-bold">SH (Fricative)</button>
                        <button onClick={()=>loadPreset('h')} className="px-3 py-1 bg-white border rounded-lg text-xs hover:bg-slate-50 font-bold">H (Breath)</button>
                    </div>
                 </div>
                 ...
                 <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-0">
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                            <h3 className="text-xs font-black text-slate-500 uppercase flex items-center gap-2"><Sliders size={14}/> 노이즈 필터 (Filter)</h3>
                            <div className="space-y-3">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input type="range" min="100" max="10000" step="10" value={frequency} onChange={e=>setFrequency(Number(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full appearance-none accent-cyan-500"/>
                                </div>
                                <div className="space-y-1">
                                ...
                                <div className="flex gap-4 pt-2">
                                    <label className="flex items-center gap-2 text-[10px] font-bold text-slate-600 cursor-pointer">
                                        <input type="radio" checked={noiseType==='white'} onChange={()=>setNoiseType('white')} className="accent-cyan-500"/> White Noise
                                    </label>
                                    <label className="flex items-center gap-2 text-[10px] font-bold text-slate-600 cursor-pointer">
                                        <input type="radio" checked={noiseType==='pink'} onChange={()=>setNoiseType('pink')} className="accent-cyan-500"/> Pink Noise
                                    </label>
                                </div>
                            </div>
                        </div>
