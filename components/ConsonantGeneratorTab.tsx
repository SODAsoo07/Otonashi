...
const GENERATOR_TEXT = {
    ko: {
        ...
        release: '릴리즈',
        outputGain: '출력 게인',
        transientOn: '트랜지언트 ON',
        generateAndPlay: '생성 및 재생',
        playing: '재생 중...',
        saveToRack: '보관함에 저장',
    },
    en: {
        ...
        transient: 'Transient',
        burstGain: 'Burst Gain',
        burstFreq: 'Burst Freq',
        burstDecay: 'Burst Decay',
        filter: 'Multi Filter',
        highpass: 'Highpass',
        bandpass: 'Bandpass',
        lowpass: 'Lowpass',
    ...
    ja: {
        ...
        release: 'リリース',
        outputGain: '出力ゲイン',
        transientOn: 'トランジェント ON',
        generateAndPlay: '生成して再生',
        playing: '再生中...',
        saveToRack: 'ラックに保存',
    },
} as const;

const ConsonantGeneratorTab: React.FC<ConsonantGeneratorTabProps> = ({ audioContext, files, onAddToRack, isActive, monitorGainValue = 1.0 }) => {
    const { language } = useLanguage();
    const text = GENERATOR_TEXT[language];
    // Envelope Params
    ...
    const handleGenerateAndPlay = useCallback(async () => {
        ...
        if (buf) {
            ...
            const source = audioContext.createBufferSource(); source.buffer = buf;
            const monitorNode = audioContext.createGain(); monitorNode.gain.value = monitorGainValue;
            source.connect(monitorNode); monitorNode.connect(audioContext.destination); source.start();
            ...
            <div className="bg-white/60 rounded-3xl border border-slate-300 p-8 flex flex-col gap-6 shadow-sm h-full overflow-y-auto custom-scrollbar">
                <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                    ...
                    <div className="flex items-center gap-2 font-black">
                        <span className="text-xs text-slate-400 font-black mr-2 uppercase tracking-tight">{text.quickPresets}:</span>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-0">
                    <div className="lg:col-span-1 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2" onMouseUp={() => commitChange()}>
                        ...
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                            <h3 className="text-sm font-black text-slate-500 uppercase flex items-center gap-2"><Mic2 size={16} /> {text.source}</h3>
                            <div className="space-y-3">
                                ...
                                {baseSource === 'file' ? (
                                    <select value={selectedFileId} onChange={e => setSelectedFileId(e.target.value)} className="w-full p-2 border rounded text-xs font-black text-slate-900"><option value="">{text.selectFile}</option>{files.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}</select>
                                ) : (
                                    <div className="space-y-3">
                                        <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.noiseMix}</span><span>{Math.round((1 - sourceMix) * 100)}%</span></div><input type="range" min="0" max="1" step="0.05" value={1 - sourceMix} onChange={e => setSourceMix(1 - Number(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full appearance-none accent-indigo-500" /></div>
                                        <div className="flex gap-2"><button onClick={() => setNoiseType('white')} className={`flex-1 py-1 text-[10px] font-black rounded border ${noiseType === 'white' ? 'bg-slate-700 text-white' : 'bg-white text-slate-500'}`}>{text.whiteNoise}</button><button onClick={() => setNoiseType('pink')} className={`flex-1 py-1 text-[10px] font-black rounded border ${noiseType === 'pink' ? 'bg-slate-700 text-white' : 'bg-white text-slate-500'}`}>{text.pinkNoise}</button></div>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className={`bg-white p-6 rounded-xl border transition-all shadow-sm space-y-4 ${transientOn ? 'border-amber-300 ring-1 ring-amber-100' : 'border-slate-200'}`}>
                            <div className="flex items-center justify-between">
                                <h3 className="text-sm font-black text-slate-500 uppercase flex items-center gap-2"><Zap size={16} className={transientOn ? "text-amber-500" : ""} /> {text.transient}</h3>
                            ...
                            {transientOn && (
                                <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.burstGain}</span><span>{Math.round(transientGain * 100)}%</span></div><input type="range" min="0" max="1.5" step="0.1" value={transientGain} onChange={e => setTransientGain(Number(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full appearance-none accent-amber-500" /></div>
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.burstFreq}</span><span>{transientFreq} Hz</span></div><input type="range" min="100" max="8000" step="100" value={transientFreq} onChange={e => setTransientFreq(Number(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full appearance-none accent-amber-500" /></div>
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.burstDecay}</span><span>{transientDecay} ms</span></div><input type="range" min="5" max="50" step="1" value={transientDecay} onChange={e => setTransientDecay(Number(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full appearance-none accent-amber-500" /></div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                            <h3 className="text-sm font-black text-slate-500 uppercase flex items-center gap-2"><Sliders size={16} /> {text.filter}</h3>
                            <div className="space-y-3">
                                <FilterControl label={text.highpass} state={hpFilter} onChange={setHpFilter} minFreq={200} />
                                <FilterControl label={text.bandpass} state={bpFilter} onChange={setBpFilter} minFreq={100} />
                                <FilterControl label={text.lowpass} state={lpFilter} onChange={setLpFilter} minFreq={500} />
                            </div>
                        </div>

                        {/* Envelope Section */}
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-5">
                            <h3 className="text-sm font-black text-slate-500 uppercase flex items-center gap-2"><Activity size={16} /> {text.envelope}</h3>
                            <div className="space-y-4">
                                <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.duration}</span><span>{duration} ms</span></div><input type="range" min="20" max="1000" value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-full appearance-none accent-indigo-500" /></div>
                                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.attack}</span><span>{attack} ms</span></div><input type="range" min="0" max="200" value={attack} onChange={e => setAttack(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-full appearance-none accent-indigo-500" /></div>
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.decay}</span><span>{decay} ms</span></div><input type="range" min="0" max="200" value={decay} onChange={e => setDecay(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-full appearance-none accent-indigo-500" /></div>
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.sustain}</span><span>{Math.round(sustain * 100)}%</span></div><input type="range" min="0" max="1" step="0.01" value={sustain} onChange={e => setSustain(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-full appearance-none accent-indigo-500" /></div>
                                    <div className="space-y-1"><div className="flex justify-between text-xs text-slate-500 font-bold"><span>{text.release}</span><span>{release} ms</span></div><input type="range" min="0" max="500" value={release} onChange={e => setRelease(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-full appearance-none accent-indigo-500" /></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-4">
                        <div className="flex-1 bg-slate-900 rounded-2xl border border-slate-700 relative overflow-hidden shadow-inner group">
                            ...
                            <div className="absolute top-4 right-4 flex flex-col items-end gap-1 font-black">
                                {transientOn && <span className="bg-amber-500/20 text-amber-400 px-2 py-1 rounded text-xs backdrop-blur font-mono border border-amber-500/30">{text.transientOn}</span>}
                        ...
                        <div className="h-24 bg-white rounded-2xl border border-slate-300 p-6 flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-6">
                                <div className="flex flex-col gap-1">
                                    <span className="text-xs font-black text-slate-400 uppercase tracking-tighter">{text.outputGain}</span>
                                    <div className="flex items-center gap-3">
