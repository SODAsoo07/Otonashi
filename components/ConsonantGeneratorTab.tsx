...
const ConsonantGeneratorTab: React.FC<ConsonantGeneratorTabProps> = ({ audioContext, files, onAddToRack, isActive, onSendToStudio, onSendToVocoder }) => {
    // Envelope Params
    ...
    const handleSave = async () => {
        ...
        onAddToRack(buffer, `자음_${timeString}`);
    };
    ...
    const handleSendToStudio = async () => {
        ...
        const date = new Date();
        const timeString = `${date.getHours().toString().padStart(2, '0')}${date.getMinutes().toString().padStart(2, '0')}`;
        onSendToStudio?.(buffer, `자음_${timeString}`);
    };
    ...
    const handleSendToVocoder = async () => {
        ...
        const date = new Date();
        const timeString = `${date.getHours().toString().padStart(2, '0')}${date.getMinutes().toString().padStart(2, '0')}`;
        onSendToVocoder?.(buffer, `자음_${timeString}`);
    };
    ...
    useEffect(() => {
        ...
        const handleKey = (e: KeyboardEvent) => {
            ...
            if (e.code === 'Space') {
                ...
                e.preventDefault();
                if (generatedBuffer) {
                    AudioUtils.downloadWav(generatedBuffer, "consonant_export.wav");
                }
            }
        };
        window.addEventListener('keydown', handleKey);
        return () => window.removeEventListener('keydown', handleKey);
    }, [isActive, handleGenerateAndPlay, handleUndo, handleSave, generatedBuffer]);
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-4">
                        ...
                        <div className="h-24 bg-white rounded-2xl border border-slate-300 p-6 flex items-center justify-between shadow-sm">
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={handleSave} className="px-6 py-4 bg-white border border-slate-300 text-slate-900 hover:bg-slate-50 rounded-xl font-black flex items-center gap-2 transition-all active:scale-95 text-sm">
                                    <Save size={18} /> 보관함
                                </button>
                                {onSendToStudio && (
                                    <button onClick={handleSendToStudio} className="px-5 py-4 bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-100 rounded-xl font-black flex items-center gap-2 transition-all active:scale-95 text-sm" title="스튜디오 탭으로 전송">
                                        → 스튜디오
                                    </button>
                                )}
                                {onSendToVocoder && (
                                    <button onClick={handleSendToVocoder} className="px-5 py-4 bg-purple-50 border border-purple-200 text-purple-700 hover:bg-purple-100 rounded-xl font-black flex items-center gap-2 transition-all active:scale-95 text-sm" title="보코더 탭으로 전송">
                                        → 보코더
                                    </button>
                                )}
                            </div>
